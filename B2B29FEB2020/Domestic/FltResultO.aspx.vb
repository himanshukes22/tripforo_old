Imports System.Data
Imports System.Xml
Partial Class FlightDom_FltResultO
            ...
            Else
                If Page.IsPostBack Then
                    ...
                    Try
                        If eventArgument.Trim.Length = 2 Then
                            faredt = FilterDtAllFlights(FilterByAirLine(Session("DomAirDt"), eventArgument), "All", True)
                        Else
                            If eventTarget.ToString.IndexOf("chkair") < 0 And eventTarget.ToString.IndexOf("sortRbdlist") < 0 Then
                                faredt = FilterDtAllFlights(Session("DomAirDt"), eventArgument, False)
                            End If
                        End If
                        If eventTarget.ToString.IndexOf("chkair") < 0 And eventTarget.ToString.IndexOf("sortRbdlist") < 0 Then
                ...
                Else
                    Try
                        ...
                        End If
                        SearchQuery.Add("FType", "OutBound")
                        SearchQuery.Add("TrackId", "")
                        Session("SearchQuery") = SearchQuery

                        DomAirDt = objDomFlt.GetResults(SearchQuery)
                    Catch ex As Exception
                        clsErrorLog.LogInfo(ex)
                    End Try
                    If DomAirDt.Rows.Count < 1 Then
                        Response.Redirect("NoResult.aspx")
                    Else
                        'xmlRes.DocumentContent = str
                        'IntAirDs.ReadXml(New System.IO.StringReader(str))
                        'IntAirDt = IntAirDs.Tables(0)
                        Session("DomAirDt") = DomAirDt
                        Dim faredt As DataTable
                        faredt = FilterDtAllFlights(DomAirDt, "All", False)
                        Dim ds1 As New DataSet
                        ds1.Tables.Add(faredt.Copy)
                        Dim MasterPricerResponse As String = ds1.GetXml()
                        xmlRes.DocumentContent = MasterPricerResponse
                        lblTotal.Text = DomAirDt.Rows(DomAirDt.Rows.Count - 1)("LineItemNumber") & "  matching flights found"
                        matrixTable(faredt)
                        If Not Page.IsPostBack Then
                            If DomAirDt.Rows.Count <> 0 Then
                                CreateDynamicControl(DomAirDt)
                            End If
                        End If
                    End If
                End If
