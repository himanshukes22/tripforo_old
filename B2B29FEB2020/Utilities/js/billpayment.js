...
function RechargeEvent() {
    ...
    if (mobileno.length == 10) {
        ...
        $.ajax({
            ...
            url: "/BillPayment.aspx/MobileRecharge",
            data: '{mobile: ' + JSON.stringify(mobileno) + ',opratorname: ' + JSON.stringify(opratorname) + ',oprator: ' + JSON.stringify(oprator) + ',circle: ' + JSON.stringify(circle) + ',amount: ' + JSON.stringify(amount) + ',isbillfetch: ' + JSON.stringify(isbillfetch) + ',rchtype: ' + JSON.stringify(txnmode) + ',fetchid:' + JSON.stringify(fetchid) + '}',
            datatype: "json",
...
$("#ddlMobileOprator").change(function () {
    ...
    if (spkey != "0") {
        $.ajax({
            type: "Post",
            contentType: "application/json; charset=utf-8",
            url: "/BillPayment.aspx/BindInputByLabelDel",
...
function DTHRechargeEvent() {
    ...
    if (dthnumber.length >= 6 && dthnumber.length <= 18) {
        ...
        DoDirectPayment("DTHPayment", spkey, spkeytext, optionsvalue, optionsids, "ddlDTHOprator", "modeldthheadbody", "dthmodelclickclass", "dthnotesmsg", $("#btnDTHRecharge"), "Recharge", fetchid);
    }
    else {
...
$("#ddlElectricityBoard").change(function () {
    ...
    if (spkey != "0") {
        ...
        $("#electricitynotesmsg").html("").addClass("hidden");
    }
});
...
function FeatchElectricityBill() {
    ...
    if (iselectricityfieled) {
        ...
        if (isbillfetch.toLowerCase() == "true") {
            ...
            DoDirectPayment("ElectricityPayment", spkey, spkeytext, optionsvalue, optionsids, "ddlElectricityBoard", "modelelectricityheadbody", "electricitymodelclickclass", "electricitynotesmsg", thisbutton, "Get Bill", fetchid);
        }
    }
}
...
function ElectricityPaymentSubmit() {
    ...
    DoPayment("ElectricityPayment", spkey, spkeytext, optionsvalue, optionsids, "ddlElectricityBoard", "modelelectricityheadbody", "electricitynotesmsg", $("#btnElectricityPayment"), "Pay Bill", fetchid);
}

//============================Start Landline Recharge Section=====================================
$("#ddlLandlineOperator").change(function () {
    $(".landlinedynamicinput").remove();
    $("#landlinenotesmsg").html("").addClass("hidden");
    var spkey = $(this).val();
...
function FeatchLandlineBill() {
    ...
    if (islandlinefieled) {
        ...
        if (isbillfetch.toLowerCase() == "true") {
            ...
            DoDirectPayment("LandlinePayment", spkey, spkeytext, optionsvalue, optionsids, "ddlLandlineOperator", "modellandlineheadbody", "landlinemodelclickclass", "landlinenotesmsg", thisbutton, "Get Bill", fetchid);
        }
    }
}
...
function LandlinePaymentSubmit() {
    ...
    DoPayment("LandlinePayment", spkey, spkeytext, optionsvalue, optionsids, "ddlLandlineOperator", "modellandlineheadbody", "landlinenotesmsg", $("#btnLandlinePayment"), "Pay Bill", fetchid);
}
...
$("#ddlInsurance").change(function () {
    ...
    $("#insurancenotesmsg").html("").addClass("hidden");
    var spkey = $(this).val();
...
function InsurancePaymentSubmit() {
    ...
    DoPayment("InsurancePayment", spkey, spkeytext, optionsvalue, optionsids, "ddlInsurance", "modelinsuranceheadbody", "insurancenotesmsg", $("#btnInsurancePayment"), "Pay Bill", fetchid);
}

//============================Start GAS Recharge Section=====================================
$("#ddlGasProvider").change(function () {
    $(".gasdynamicinput").remove();
    $("#gasnotesmsg").html("").addClass("hidden");
    var spkey = $(this).val();
...
function GasPaymentSubmit() {
    ...
    DoPayment("GasPayment", spkey, spkeytext, optionsvalue, optionsids, "ddlGasProvider", "modelgasheadbody", "gasnotesmsg", $("#btnGasPayment"), "Pay Bill", fetchid);
}

//============================Start Broadband Recharge Section=====================================
$("#ddlInternetService").change(function () {
    $(".internetdynamicinput").remove();
    $("#internetnotesmsg").html("").addClass("hidden"); 
...
function BroadbandPaymentSubmit() {
    ...
    DoPayment("BroadbandPayment", spkey, spkeytext, optionsvalue, optionsids, "ddlInternetService", "modelinternetheadbody", "internetnotesmsg", $("#btnBroadbandPayment"), "Pay Bill", fetchid);
}

//============================Start Water Recharge Section=====================================
$("#ddlWaterBoard").change(function () {
    $(".waterdynamicinput").remove();
    $("#waternotesmsg").html("").addClass("hidden");
...
function WaterPaymentSubmit() {
    ...
    DoPayment("WaterPayment", spkey, spkeytext, optionsvalue, optionsids, "ddlWaterBoard", "modelwaterheadbody", "waternotesmsg", $("#btnWaterPayment"), "Pay Bill", fetchid);
}
...
$(".actiontype").click(function () {
    ...
    if (actionType == "electricity") { $("#electricitynotesmsg").html("").addClass("hidden"); $("#ElectricityAmount").val("").addClass("hidden"); BindOpratorByType("ddlElectricityBoard", "ELECTRICITY", 'Electricity Board'); $(".electricitydynamicinput").remove();   }
    if (actionType == "landline") { $("#landlinenotesmsg").html("").addClass("hidden"); $(".landlinedynamicinput").remove(); BindOpratorByType("ddlLandlineOperator", "LANDLINE", 'Operator'); }
    if (actionType == "insurance") { $("#insurancenotesmsg").html("").addClass("hidden"); $(".insurancedynamicinput").remove(); BindOpratorByType("ddlInsurance", "INSURANCE", 'Insurer'); }
    if (actionType == "gas") { $("#gasnotesmsg").html("").addClass("hidden"); $(".gasdynamicinput").remove(); BindOpratorByType("ddlGasProvider", "GAS", 'Gas Provider'); }
    ...
    if (actionType == "water") { $("#waternotesmsg").html("").addClass("hidden"); $(".waterdynamicinput").remove(); BindOpratorByType("ddlWaterBoard", "WATER", 'Board'); }
...
function BindOpratorByType(dropdownid, servicetype, selectname) {
    ...
    $.ajax({
        type: "Post",
        url: "/BillPayment.aspx/BindOpratorByType",
...
function DoPayment(poststr, spkey, spkeytext, optionsvalue, optionsids, ddldropdown, modelbody, notesmsg, thisbutton, thisbuttontext, fetchid) {
    $(thisbutton).html("Please Wait... <i class='fa fa-pulse fa-spinner'></i>").prop('disabled', true);
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        url: "/BillPayment.aspx/" + poststr,
        data: '{spkeytext: ' + JSON.stringify(spkeytext) + ',spkey: ' + JSON.stringify(spkey) + ',optionsvalue: ' + JSON.stringify(optionsvalue) + ',fetchid: ' + JSON.stringify(fetchid) + '}',
        datatype: "json",
        success: function (data) {
            if (data.d != null) {
                //$(".closepopup").click();
...
function DoDirectPayment(poststr, spkey, spkeytext, optionsvalue, optionsids, ddldropdown, modelbody, modelclassclick, notesmsg, thisbutton, thisbuttontext, fetchid) {
    $(thisbutton).html("Please Wait... <i class='fa fa-pulse fa-spinner'></i>").prop('disabled', true);
...
function RefundBillFaildAmount(transid) {
    ...
    if (reportid != "") {
        ...
        $.ajax({
            type: "Post",
            contentType: "application/json; charset=utf-8",
            url: "/BillPayment.aspx/ProcessBillToReFund",
...
function CallBackCheckStatus(billid) {
    ...
    if (transid != "" && clientrefid != "") {
        ...
        $.ajax({
            type: "Post",
            contentType: "application/json; charset=utf-8",
            url: "/BillPayment.aspx/ProcessToCallBackCheckStatus",
