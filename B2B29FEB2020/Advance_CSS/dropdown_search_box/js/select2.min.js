...
Autocomplete.prototype.createTextBox = function () {
	...
	//this.shortcutLabel.classList.add('fa fa-search', 'form-control-feedback');
	...
	this.textBox.setAttribute('placeholder', 'Search');
...
Autocomplete.prototype.createShortcuts = function () {
	...
	this.shortcutWrapper = document.createElement('div');
	//this.shortcutWrapper.setAttribute('tabIndex', '0');
	this.shortcutWrapper.classList.add('tab__list');
	...
	for (var i = 0; i < this.shortcuts.length; i++) {
		...
		button.classList.add('tab__item');
...
$("#avlsector_id").css("display", "none");
$.ajax({
	url: UrlBase + "FltSearch1.asmx/GetFDDSectorDetails",
	data: "{ 'dep': '','arr': '' }",
	dataType: "json",
	type: "POST",
	...
	success: function (response) {
		if (response.d != "" && response.d != undefined) {
			$("#avlsector_id").css("display", "block");
			BindSectorInDropDown(response.d);
		}
		else {
			$("#avlsector_id").css("display", "none");
			$("#div_LatestOffers").css("display","none");
		}
	}
});


BindSectorInDropDown = (arrlist) => {
	debugger;
	// vars
	const dropdown = document.querySelector('#field-dropdown');

	const names = arrlist.slice(0, (arrlist.length - 1));;
	let nums = [];
	let recentNames = [];
	...
	var lastArr = arrlist.pop();
	$("#BindDepArrDetails").html(lastArr);
...
$.ajax({
	url: UrlBase + "FltSearch1.asmx/GetFDDOfferExpireSoonDetails",
	//data: "{ 'dep': '','arr': '' }",
	dataType: "json",
	type: "POST",
	contentType: "application/json; charset=utf-8",
	success: function (response) {
		if (response.d != "" && response.d != undefined) {
			$(".divLimitedOfferExpireSoon").html(response.d);
		}
		else {
			alert(response.d);
		}
...
//const names = ['Delhi // Mumbai', 'Mumbai // Delhi', 'Delhi // Patna', 'Patna // Delhi', 'Ahmedabad // Delhi', 'Delhi // Ahmedabad'];
//let nums = [];
//let recentNames = [];
