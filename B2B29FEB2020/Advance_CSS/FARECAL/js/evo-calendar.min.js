...
! function (e) {
    ...
    i = 0, (r = function (e, t) {
        ...
        var e = this;
        let currmonth = e.$active.month;
        if (e.calculateDays(), !e.$elements.calendarEl.html()) {
            //for (var t = '<div class="calendar-sidebar"><div class="calendar-year"><a class="icon-button" role="button" data-year-val="prev" title="' + e.initials.dates[e.options.language].previousYearText + '"><span class="icofont-dotted-left icofont-2x" style="color: #9ad75a;"></span></a>&nbsp;<p></p>&nbsp;<a class="icon-button" role="button" data-year-val="next" title="' + e.initials.dates[e.options.language].nextYearText + '"><span class="icofont-dotted-right icofont-2x" style="color: #9ad75a;"></span></a></div><div class="month-list"><ul class="calendar-months">', a = 0; a < e.$label.months.length; a++) t += '<li ' + (a < currmonth ? "style='color: #777;cursor:no-drop;'" : "class='month' role='button' data-month-val='" + a + "'") + '>' + e.initials.dates[e.options.language].months[a] + "</li>";
			for (var t = '<i class="icofont-close-squared-alt" id="CloseCal" title="Close Calander" style="float: right;position: absolute;right: 0;z-index: 99999;color: #9ad75a;font-size: 25px;cursor:pointer;"></i><div class="calendar-sidebar"><div class="calendar-year"><a class="icon-button" role="button" data-year-val="prev" title="' + e.initials.dates[e.options.language].previousYearText + '"><span class="icofont-dotted-left icofont-2x" style="color: #9ad75a;"></span></a>&nbsp;<p></p>&nbsp;<a class="icon-button" role="button" data-year-val="next" title="' + e.initials.dates[e.options.language].nextYearText + '"><span class="icofont-dotted-right icofont-2x" style="color: #9ad75a;"></span></a></div><div class="month-list"><ul class="calendar-months">', a = 0; a < e.$label.months.length; a++) t += '<li ' + (a < currmonth ? "style='color: #777;cursor:no-drop;'" : "class='month' role='button' data-month-val='" + a + "'") + '>' + e.initials.dates[e.options.language].months[a] + "</li>";
            ...
            t += "</tr></table></div>", t += '<div class="calendar-events"><div class="event-header"><p></p></div><div class="event-list"></div></div>', e.$elements.calendarEl.html(t), e.$elements.sidebarEl || (e.$elements.sidebarEl = d(e.$elements.calendarEl).find(".calendar-sidebar")), e.$elements.innerEl || (e.$elements.innerEl = d(e.$elements.calendarEl).find(".calendar-inner")), e.$elements.eventEl || (e.$elements.eventEl = d(e.$elements.calendarEl).find(".calendar-events")), e.options.sidebarToggler && (d(e.$elements.sidebarEl).append('<span id="sidebarToggler" role="button" aria-pressed title="' + e.initials.dates[e.options.language].closeSidebarText + '"><a class="icon-button"><span class="icofont-navigation-menu" style="color:#fff !important"></span></a></span>'), e.$elements.sidebarToggler || (e.$elements.sidebarToggler = d(e.$elements.sidebarEl).find("span#sidebarToggler"))), e.options.eventListToggler && (d(e.$elements.calendarEl).append('<span id="eventListToggler" role="button" aria-pressed title="' + e.initials.dates[e.options.language].closeEventListText + '"><a class="icon-button"><span class="icofont-airplane icofont-rotate-90"></span></a></span>'), e.$elements.eventListToggler || (e.$elements.eventListToggler = d(e.$elements.calendarEl).find("span#eventListToggler")))
        }
		//month bind
        let temp_firstMonth =e.options.calendarEvents[0].date; //e.options.calendarEvents[e.options.calendarEvents.length - 1].date;
         let temp_converteddate = new Date(temp_firstMonth);
         let temp_month = $.datepicker.formatDate('M', temp_converteddate);
         let temp_month_number = ("JanFebMarAprMayJunJulAugSepOctNovDec".indexOf(temp_month) / 3 + 1) - 1;
        // e.$active.month = temp_month_number; e.$active.date = temp_firstMonth; e.$active.event_date = temp_firstMonth; e.$current.month = temp_month_number; e.$current.date = temp_firstMonth;
		e.$active.month = temp_month_number; e.$current.month = temp_month_number; e.$current.date = temp_firstMonth;
        ...
        t.$elements.eventEl.find(".event-header > p").text(CustomDateFormate(n));//krishna
        ...
        function CustomDateFormate(strdate) {
        ...
        srla += "<div class='col-md-3 col-xs-3'><p class='dep-loc'>" + e.DepartureLocation + " <span class='dep-time'><p>" + e.Departure_Date + "</p>" + e.DepartureTime + "</span></p>" + e.Stops + "</div>";
        srla += "<div class='col-md-3 col-xs-3'> <p class='arr-loc'> " + e.ArrivalLocation + " <span class='arr-time'><p>" + e.Arrival_Date + "</p>" + e.ArrivalTime + " </span></p>" + e.TotDur + "</div>";
        var Refd = e.AdtFareType.trim().toLowerCase() == "non refundable" ? "NR" : "R";
        if (e.FareDet != "")
            srla += "<div class='col-md-3 col-xs-3' style='padding-right: 5px;padding-left: 5px;'> <p class='tot-fair'>" + e.FareDet + e.TotalFare + " </p>  <p class='btnbookf' title='" + e.LineNumber + "'>Book</p></div>";  //" + e.AdtFar + "
        else
            srla += "<div class='col-md-3 col-xs-3' style='padding-right: 5px;padding-left: 5px;'> <p class='tot-fair'>₹ " + e.TotalFare + " </p>  <p class='btnbookf' title='" + e.LineNumber + "'>Book</p></div>";  //" + e.AdtFar + "
        srla += "<div class='flt-information'><span class='f-type' title='" + e.AdtFareType + "'>" + Refd + "</span>  <i class='icofont-dotted-right'></i> <span><i title='Available Seat'><img src='/Advance_CSS/FARECAL/js/seat.png' style='width:20px;'/></i> (" + e.AvailableSeats1 + ")</span> <i class='icofont-dotted-right'></i> <span><i class='icofont-info-circle flt-info' title='Flight Details' id ='flt-info' rel=' " + e.LineNumber + "'></i></span></div>";

        srla += "</div>";
        srla += "";
        srla += "";

        0 === n.find("[data-event-index]").length && n.empty(), a.$active.events.push(e), t = '<div class="event-container"  role="button" data-event-index="' + e.id + '">' + srla + '', t += '<div class="event-bullet-' + e.type + '"', e.color && (t += 'style="background-color:' + e.color + '"'), t += '></div></div><div class="event-info"><p class="event-title hide">' + a.limitTitle(e.name), e.badge && (t += "<span>" + e.badge + "</span>"), t += "</p>", e.description && (t += '<p class="event-desc hide">' + e.description + "</p>"), t += "</div>", t += "</div>", n.append(t), a.$elements.eventEl.find('[data-event-index="' + e.id + '"]').off("click.evocalendar").on("click.evocalendar", a.selectEvent)
        ...
        let converteddate = new Date(t.$active.date);
        let currday = $.datepicker.formatDate('dd', converteddate);
        let currtextmon = $.datepicker.formatDate('MM', converteddate);
        ...
        for (var a = 1, n = 0; n < 9; n++) {
            for (var i, o = 0; o < t.$label.days.length; o++) {
                if (currtextmon == r) {
                    a <= t.monthLength && (0 < n || o >= t.startingDay) ? (i = "calendar-day", t.$label.days[o] !== t.initials.weekends.sat && t.$label.days[o] !== t.initials.weekends.sun || (i += " --weekend"), e += '<td class="' + (a < currday ? "calendar-daynot" : i) + '">', e += '<div ' + (a < currday ? "class='daynot'" : "class='day' role='button' data-date-val='" + t.formatDate(t.$label.months[t.$active.month] + " " + a + " " + t.$active.year, t.options.format) + "'") + '>' + a + "</div>", a++) : e += "<td>", e += "</td>"
                }
                else {
        ...
        function d(e) {
            ...
            if (t.FareDet != "") {
                0 === (n = i.$elements.innerEl.find('[data-date-val="' + e + '"]')).find("span.event-indicator").length && n.append('<span class="event-indicator"></div></span>'), 0 === n.find("span.event-indicator > .type-bullet > .type-" + o).length && (a = '<div class="type-bullet"><div ', a += 'class="type-' + t.type + '"', t.color && (a += 'style="background-color:' + t.color + '"'), a += "><div class='T-fare'>" + t.FareDet+ t.fare + "</div></div>", n.find(".event-indicator").append(a))
            }
            else {
                0 === (n = i.$elements.innerEl.find('[data-date-val="' + e + '"]')).find("span.event-indicator").length && n.append('<span class="event-indicator"></div></span>'), 0 === n.find("span.event-indicator > .type-bullet > .type-" + o).length && (a = '<div class="type-bullet"><div ', a += 'class="type-' + t.type + '"', t.color && (a += 'style="background-color:' + t.color + '"'), a += "><div class='T-fare'>₹ " + t.fare + "</div></div>", n.find(".event-indicator").append(a))
            }
        }
