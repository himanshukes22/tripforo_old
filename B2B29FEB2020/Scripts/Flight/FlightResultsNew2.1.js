...
var RTFFirstTime = 0;
var RTFFirstTimeForSlider = 0;

var gdsJsonRTFView = new Array();
var airlineNameRTF = "";
var airlineNameRTFAll = new Array();
var SRTFFilter = true;
//Devesh SRF END
...
var ResHelper = function () {
   ...
   // this.stopFilterRTF = $("#stopFlterRTF");
    this.flterRTF = $(".FoRTF");
   // this.AirlineFareTypeRTF = $("#AirlineFareTypeRTF");
...
ResHelper.prototype.BindEvents = function () {
    ...
    e.eventFn();
    e.NextPrevSearch(e);
    e.ShowFareBreakUp();            // Add  By Devesh
    e.GetFltDetails();            // Add  By Devesh
};
...
ResHelper.prototype.eventFn = function () {
    ...
    e.lccRTFDiv.click(function () {
        ...
        if (lccRtfResult == null) {
            ...
            //e.GetFltDetails(lccRtfResultArray);         // Coment By Devesh-25-04-2018
            //e.ShowFareBreakUp(lccRtfResultArray);     // Coment By Devesh-25-04-2018
    ...
    e.gdsRTFDiv.click(function () {
        ...
        if (gdsRtfResult == null) {
            ...
            //e.GetFltDetails(gdsRtfResultArray);          // Coment By Devesh-25-04-2018
            // e.ShowFareBreakUp(gdsRtfResultArray);     // Coment By Devesh-25-04-2018
...
ResHelper.prototype.Book = function (result) {
    ...
    e.bookO.click(function () {        
        ...
        for (var i = 0; i < arr[0].length; i++) {
            if (arr[0][i].LineNumber.toString().indexOf('ITZ') >= 0) {
            ...
            if (arr[0][i].LineNumber.toString().indexOf('api') >= 0) {
...
//ResHelper.prototype.ShowFareBreakUp = function (result) {
    ResHelper.prototype.ShowFareBreakUp = function () {

    var t = this;
    this.gridViewToolTip = $(".gridViewToolTip");

    $("#MainSFR").delegate(".gridViewToolTip", "click", function (event) {
    //t.gridViewToolTip.click(function (event) {
        ...
        if (lineNum[1] == "R") {
            fltSelectedArray = JSLINQ(CommonResultArray[1])
            ...
            if (lineNum.indexOf('RTF') >= 0 && lineNum.length > 2 && lineNum[2]=="RTF")
            {
               // lineNum[3]   //Airline Code
            //    var fltArray = JSLINQ(gdsJsonRTF)
            //.Where(function (item) { return $.trim(item.ValiDatingCarrier).replace(/\s/g, '') == lineNum[3]; })
                //.Select(function (item) { return item.fltJson; });

                var fltArray = JSLINQ(gdsJsonRTF)
           .Where(function (item) { return $.trim(item.fltName).replace(/\s/g, '') == lineNum[4]; })
           .Select(function (item) { return item.fltJson; });

                //var res = new Array();
                //res.push({ fltsearch1: fltArray.items[0] });

                var res = new Array();
                res.push({ fltsearch1: fltArray.items[0], fltName: lineNum[4] });

                var res1 = new Array();
                res1.push(res[0].fltsearch1);
                //fltSelectedArray = res1;

                fltSelectedArray = JSLINQ(res1[0])
                                            .Where(function (item) { return item.LineNumber == lineNum[0]; })
                                            .Select(function (item) { return item });
                ...
                //                            .Where(function (item) { return item.LineNumber == lineNum[0]; })
                //                            .Select(function (item) { return item });
                fltSelectedArray = JSLINQ(CommonResultArray[0])
                                           .Where(function (item) { return item.LineNumber == lineNum[0]; })
                                           .Select(function (item) { return item });
            }
   ...
   // $('.fareRuleToolTip').click(function (event) {
    $("#MainSFR").delegate(".fareRuleToolTip", "click", function (event) {
        event.preventDefault();
        var th = this;
        var lineNum;// = $(th).next().attr('title').split('_');
        var lineNums = $(th).next().attr('title').split('_');
        if (lineNums != null) {
        if (lineNums.indexOf('RTF') >= 0 && lineNums.length > 2 && lineNums[2] == "RTF") {
            lineNum = $(th).next().attr('title').split('_');
            lineNum1 = $(th).next().attr('title');
            lineNum[0] = lineNum1;
        }
        else {
            lineNum = $(th).next().attr('title').split('_');
        ...
        if (lineNum[0] != null) {
            ...
            if (lineNum[1] == "R") {
                ...
                if (lineNums.indexOf('RTF') >= 0 && lineNums.length > 2 && lineNums[2] == "RTF") {
                    ...
                    var fltArray = JSLINQ(gdsJsonRTF)
               .Where(function (item) { return $.trim(item.fltName).replace(/\s/g, '') == lineNums[4]; })
               .Select(function (item) { return item.fltJson; });                    

                    var res = new Array();
                    res.push({ fltsearch1: fltArray.items[0], fltName: lineNums[4] });
    ...
    $("#MainSFR").delegate(".fltfareDetailsR", "click", function (event) {
           ...
           .Select(function (item) { return item });
            ...
            if (O.items.length > 0 || R.items.length > 0) {
                ...
                $.ajax({
                    ...
                    error: function (e, t, n) {
                     DiplayMsearch($.trim($(th).next()[0].id));
                        alert("Please try after sometime!!")
...
ResHelper.prototype.GetStopFilterSRTF = function (result, cls, type,AirCode) {
    ...
    var str = '<div class="jplist-group" data-control-type="Stopfilter' + type + AirCode + '" data-control-action="filter"  data-control-name="Stopfilter' + type + AirCode + '"';
    str += ' data-path=".' + cls + '" data-logic="or">'

    for (var i = 0; i < marray.length; i++) {

        str += '<div class="clear"> </div><div class="lft w8"> <input value="' + marray[i] + '"  id="CheckboxS' + type + AirCode + i + 1 + '"  type="checkbox"  /> </div><div class="lft w80" style="padding-top:3px"> <label for="' + marray[i] + '">' + marray[i] + '</label></div>';
...
ResHelper.prototype.GetUniqueAirlineSRTF = function (result, cls, type,AirCode) {
    var e = this;
    var marray = new Array();
    var OF = JSLINQ(result)
          .OrderBy(function (item) { return item.AirLineName })
    ...
    var str = '<div class="jplist-group" data-control-type="Airlinefilter' + type + AirCode + '" data-control-action="filter"  data-control-name="Airlinefilter' + type + AirCode + '"';
    str += ' data-path=".' + cls + '" data-logic="or">'

    for (var i = 0; i < marray.length; i++) {


        str += '<div class="lft w8"> <input value="' + marray[i] + '"  id="CheckboxA' + type + AirCode + i + 1 + '"  type="checkbox"  /> </div><div class="lft w80" style="padding-top:3px;"><label for="' + marray[i] + '">' + marray[i] + '</label> </div> <div class="clear"> </div>';

    }
    str += '</div>';
    $('#airlineFilterRTF' + AirCode).html(str)
   // airlineFilterRTFG8
    //if (type == 'O') {
    //    e.airlineFilter.html(str);
    //}
...
ResHelper.prototype.GetFltDetails = function () {
    var e = this;
    //$('.fltDetailslink').click(function (event) {
        $("#MainSFR").delegate(".fltDetailslink", "click", function (event) {
        event.preventDefault();
        ...
        if (this.rel != null) {
            ...
            $('#' + this.rel + '_').slideUp();
            var lineNums ;//= this.rel.split('_')[0]; //.split('_');
            var lineNum = this.rel.split('_');


            //if (lineNum.indexOf('RTF') >= 0 && lineNum.length > 2 && lineNum[2] == "RTF") {
            //    $('#' + this.rel.split('_')[0] + lineNum[3] + '_').slideUp();
            //}
            //else {
            //    $('#' + this.rel.split('_')[0] + '_').slideUp();
            //}            
            var OB;
            var O;
            var R;
   ...
   // $('.fltBagDetails').click(function (event) {
        $("#MainSFR").delegate(".fltBagDetails", "click", function (event) {
    ...
    if (applyfilterStatus == false)
    {
        if (roundtripNrml1 == true) {
            ...
            if (CommonResultArray[0].length > 0 && CommonResultArray[0].length > 0) {
                ...
                t.RTFFinalBook();
              //  t.ShowFareBreakUp(CommonResultArray);  // Coment By Devesh-25-04-2018
            ...
            if (CommonResultArray[0].length > 0) {
                ...
                //t.GetFltDetails(CommonResultArray);   // Coment By Devesh-25-04-2018
                //t.ShowFareBreakUp(CommonResultArray);       // Coment By Devesh-25-04-2018
                    ...
                    if (IsNRMLRoundTrip == true) {
                        ...
                        if (resultG1.length > 0 && resultG1[0].length > 0) {
                            ...
                            if ($.trim(resultG1[0][0].Sector).length > 8 && c.TripType1 != "rdbMultiCity" && $.trim(resultG1[0][0].Trip).toUpperCase() == "D") {
                                ...
                                try {
                                    var i = 0;
                                    //var airItemRTF = { "fltName": content[i].fltName, "fltJson": content[i].fltsearch1 };
                                    var airItemRTF = { "fltName": resultG1[0][0].AirLineName, "fltJson": resultG1[0] };
                                    gdsJsonRTF.push(airItemRTF);
                                    //if (content[i].fltsearch1 != null && content[i].fltsearch1[0].length > 0) {
                                    GetUniqueAirlineRtf($.trim(resultG1[0][0].AirLineName), 0, $.trim(resultG1[0][0].AirLineName).replace(/\s/g, '').split('_')[0])
                                    //GetUniqueAirlineRtf($.trim(content[i].fltName), 0, $.trim(content[i].fltName).replace(/\s/g, '').split('_')[0])
                                    //}

                                    // stage = stage + 1;
                                    //if (stage == resultCount) {
                                    $.unblockUI();   //  Not found in Result.aspx-   Devesh
                                    //  divSort(""); //  Not found in Result.aspx-   Devesh                                  

                                    //NoResultMessage(1);   //Devesh
                                    $('#ModiFySearch').show();
                                    //$('#ModiFySearchD').hide();
        ...
        success: function (data) {
            ...
            if (resultRTFSp == "" || resultRTFSp == null) {
                ...
                if (resultRTFSp[0] == "" || resultRTFSp[0] == null) {
                    ...
                    t.Book(resultRTFSp);
                    //t.GetFltDetails(resultRTFSp);// Coment By Devesh-25-04-2018
                    // t.ShowFareBreakUp(resultRTFSp);// Coment By Devesh-25-04-2018
...
ResHelper.prototype.GetResultO = function (resultArray, maxLineNumber, Provider) {
    ...
    if (resultArray[0].length > 0) {
        ...
        for (var i = 1; i <= LnOb; i++) {
            var OB ;
            if (Provider == "RTFSPECIAL") {
                 OB = JSLINQ(resultArray[0])
            ...
            if (OB.items.length > 0) {
               ...
               .Select(function (item) { return item });
                ...
                if (OF.items.length > 0) {
                    ...
                    if (Provider == "RTFSPECIAL") {
                        result += '<div class="gridViewToolTip1 hide"  title="' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '" >ss</div>';
                        result += '<div><span class="fltDetailslinkR cursorpointer lft" rel="fltdtls_' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '" ><img src="' + UrlBase + 'images/icons/information.png" class="cursorpointer" alt="Flight Details" title="Flight Details"   /> &nbsp;</span><div class="fade" id="fltdtls_' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '" > &nbsp;</div></div>';
                        result += '<div class="fltBagDetailsR lft" rel="fltdtls_' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '"><img src="../images/icons/baggage.png"   alt="Baggages Allowed"  class="cursorpointer"title="Baggages Allowed"></div>';
                ...
                if (OF.items.length > 0) {
                    ...
                    if (Provider == "RTFSPECIAL") {
                        result += '<div class="gridViewToolTip1 hide"  title="' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '" >ss</div>';
                        result += '<div class="lft"><a href="#" id="' + OB.items[0].LineNumber + OB.items[0].ValiDatingCarrier + 'Det" rel="' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '"  class="fltDetailslink"><img src="' + UrlBase + 'images/icons/information.png" class="cursorpointer" alt="Flight Details" title="Flight Details"   /></a></div>';
                        result += '<div class="lft">&nbsp;<a href="#" id="' + OB.items[0].LineNumber + OB.items[0].ValiDatingCarrier + 'BagDet" rel="' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '"  class="fltBagDetails"> <img src="../images/icons/baggage.png"   alt="Baggages Allowed"  class="cursorpointer"title="Baggages Allowed"></a></div>';
                        ...
                        if (OB.items[0].ValiDatingCarrier != 'SG' && OB.items[0].ValiDatingCarrier != '6E' && OB.items[0].ValiDatingCarrier != 'G8')
                            result += '<span class="fareRuleToolTip cursorpointer lft" rel="FareRule_' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '" ><img src="' + UrlBase + 'images/fare-rules.png"  class="cursorpointer" alt="Fare Rule" title="Fare Rule" style="cursor:pointer;" /> &nbsp;</span><div class="fade"  title="' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '" > &nbsp;</div>';
                ...
                if (Provider == "RTFSPECIAL") {
                    //result += '<div id="' + OB.items[0].LineNumber.toString() + OB.items[0].ValiDatingCarrier + '_" style="display:none;width:100%;"></div>';
                    result += '<div id="' + OB.items[0].LineNumber + '_O_RTF_' + OB.items[0].ValiDatingCarrier + '_' + $.trim(OB.items[0].AirLineName).replace(/\s/g, '') + '_" style="display:none;width:100%;"></div>';
...
//Special Fare Filter Devesh  19-04-2018
ResHelper.prototype.FltrSortRTF = function (result,RtfAirlineName) {
    $('#IdFareType').hide();
    var RTFAircode = (result[0][0].ValiDatingCarrier).toUpperCase();
    WriteFilter(result[0][0].ValiDatingCarrier, $.trim(result[0][0].AirLineName).replace(/\s/g, '').split('_')[0]);
    this.GetStopFilterSRTF(result[0], 'stops', 'RTF',RTFAircode);
    this.GetUniqueAirlineSRTF(result[0], 'airlineImage', 'RTF', RTFAircode);
    this.GetUniqueAirlineFareTypeSRTF(result[0], 'AirlineFareType', 'RTF',RTFAircode);
    var mPr = this.GetMinMaxPrice(result[0]);
    var mT = this.GetMinMaxTime(result[0]);
    var e = this;
    $(".closeopen").next().slideDown();
    ...
    else if (RTFAircode == "6E")
    {
        e.MainSF.jplist({
            debug: false,
            itemsBox: '.listIndigo_Indigo'
                           , itemPath: '.list-item'
                           , panelPath: '.jplist-panel'
                           , storage: '' //'', 'cookies', 'localstorage'			
            //, storageName: 'jplist-list-grid'
    ...
    else if (RTFAircode == "SG") {
        e.MainSF.jplist({
            debug: false,
            itemsBox: '.listSpiceJet_SpiceJet'
                           , itemPath: '.list-item'
                           , panelPath: '.jplist-panel'
                           , storage: '' //'', 'cookies', 'localstorage'			
            //, storageName: 'jplist-list-grid'
    ...
    else if (RTFAircode == "AI") {
        e.MainSF.jplist({
            debug: false,
            itemsBox: '.listAirIndia_AirIndia'
                           , itemPath: '.list-item'
                           , panelPath: '.jplist-panel'
                           , storage: '' //'', 'cookies', 'localstorage'			
            //, storageName: 'jplist-list-grid'
    ...
    else if (RTFAircode == "UK") {
        e.MainSF.jplist({
            debug: false,
            itemsBox: '.listVistara_Vistara'
                           , itemPath: '.list-item'
                           , panelPath: '.jplist-panel'
                           , storage: '' //'', 'cookies', 'localstorage'			
            //, storageName: 'jplist-list-grid'
    ...
    else if (RTFAircode == "9W") {
        e.MainSF.jplist({
            debug: false,
            itemsBox: '.listJetAirways_JetAirways'
                           , itemPath: '.list-item'
                           , panelPath: '.jplist-panel'
                           , storage: '' //'', 'cookies', 'localstorage'			
            //, storageName: 'jplist-list-grid'
...
//Filter by Fare Type wise  - Devesh

ResHelper.prototype.GetUniqueAirlineFareTypeSRTF = function (result, cls, type, AirCode) {
    //debugger;
    var e = this;
    var marray = new Array();
    var OF = JSLINQ(result)
    ...
    var str = '<div class="jplist-group" data-control-type="AirlineFareType' + type + AirCode + '" data-control-action="filter"  data-control-name="AirlineFareType' + type + AirCode + '"';
    str += ' data-path=".' + cls + '" data-logic="or">'
    var CouponFareF = true;
    var SpecialFareF = true;
    var NormalFareF = true;
    for (var i = 0; i < marray.length; i++) {
        ...
        if (FareTypeS1 != "") {
            ...
            //SpecialFare = false;
            //NormalFareF == false;
            str += '<div class="lft w8"> <input value="' + FareTypeS1 + '"  id="CheckboxA' + type + AirCode + i + 1 + '"  type="checkbox"  /> </div><div class="lft w80" style="padding-top:3px;"><label for="' + FareTypeS1 + '">' + FilterFareTypeShow + '</label> </div> <div class="clear"> </div>';
            //str += '<div class="lft w8"> <input value="' + marray[i] + '"  id="CheckboxA' + type + i + 1 + '"  type="checkbox"  /> </div><div class="lft w80" style="padding-top:3px;"><label for="' + marray[i] + '">' + FilterFareTypeShow + '</label> </div> <div class="clear"> </div>';
        }
    ...
    if (type == 'RTF') {
       // e.AirlineFareTypeRTF.html(str)
        $('#AirlineFareTypeRTF' + AirCode).html(str)
...
//End  Filter by Fare Type wise  - Devesh
...
//Devesh
function RTFSH(status) {    
    if (status == "1") {
        isRtfFareShown = 1;
        $('#fltselct').css("display", "none");
        $('#RoundTripH').css("display", "none");
        $('#onewayH').css("display", "block");
        $('#flterTab').hide();
        $('.slideroneway').hide();
        $('#Sliderrtf').show();
        $('.flterone').hide();
       ...
       // $('#FBFT1').hide();
        ...
        //$('#AirlineFareType').hide();
        //$('#AirlineFareTypeR').hide();      
        ...
        isRtfFareShown = 0;
        $('#fltselct').css("display", "block");
        $('#RoundTripH').css("display", "block");
        $('#onewayH').css("display", "none");
        ...
        //$('#AirlineFareType').show();
        //$('#AirlineFareTypeR').show();
...
function GetUniqueAirlineRtf(result, st, svcName) {
    ...
    $('#splLoading').hide();
    if (RTFFirstTime == 0) {
        $('#RTFSAir').html('<div id="DivNormalfare" onclick="showRtfFare(\'DivNormalfare\');" class="curve rtfpanel1 bld w15">Normal Fare</div>');
        RTFFirstTime = 1;
    ...
    try {
        if (st == 1) {
            var fltArray = JSLINQ(result)
                         .Select(function (item) { return item.AirLineName; });
            var uairline = new Array()
            uairline = fltArray.items;
            var uairline1 = uairline.unique();

            for (var i = 0; i < uairline1.length; i++)
            {
                // var str = "<div id='" + uairline1[i].replace(/\s/g, '') + "_" + svcName + "' class='showrtf'  >" + uairline1[i] +"(INR "+ getMinPriceRTF(gdsJsonRTFView, uairline1[i].replace(/\s/g, '') + "_" + svcName ) + ")</div>";
                var str = "<div  id='" + uairline1[i].replace(/\s/g, '') + "_" + svcName + "'  class='curve rtfpanel mrgt10'  onclick='showRtfFare(\"" + uairline1[i].replace(/\s/g, '') + "_" + svcName + "\");'   ><img src='" + getAirImagePath(uairline1[i].replace(/\s/g, '')) + "' alt='" + uairline1[i] + "' title='" + uairline1[i] + "' class='w29 lft' /> </div> <span class='lft f16'> " + getMinPriceRTF(gdsJsonRTF, uairline1[i].replace(/\s/g, '') + "_" + svcName) + "</span></div>";
                $('#RTFSAir').append(str);
                // RTF RESULT BIND -DEVESH
                var divSRTF = '<div id="divResultRTF_' + uairline1[i].replace(/\s/g, '') + "_" + svcName + '" class="listRTF list' + uairline1[i].replace(/\s/g, '') + "_" + svcName + '" style="display:none;">';
                $('#mainDiv').append(divSRTF);
                var RtfFlightName = uairline1[i].replace(/\s/g, '') + "_" + svcName;
                GetRTFResult(gdsJsonRTF, RtfFlightName);
                // RTF RESULT BIND -DEVESH
        ...
        {
            // var str = "<div id='" + result.replace(/\s/g, '').split('_')[0] + "_" + svcName + "'  class='showrtf'>" + result.split('_')[0] +"(INR "+ getMinPriceRTF(gdsJsonRTFView, result.replace(/\s/g, '').split('_')[0] + "_" + svcName) + ")</div>";
            var str = "<div  id='" + result.replace(/\s/g, '').split('_')[0] + "_" + svcName + "' class='curve rtfpanel'  onclick='showRtfFare(\"" + result.replace(/\s/g, '').split('_')[0] + "_" + svcName + "\");' ><img src='" + getAirImagePath(result.replace(/\s/g, '').split('_')[0]) + "' title='" + result.split('_')[0] + "' class='w29 lft' alt='" + result.split('_')[0] + "' /> <span class='lft f16'> " + getMinPriceRTF(gdsJsonRTF, result.replace(/\s/g, '').split('_')[0] + "_" + svcName) + "</span></div>";
            $('#RTFSAir').append(str);
            // RTF RESULT BIND -DEVESH
            ...
            $('#mainDiv').append(divSRTF);
            var RtfFlightName = result.replace(/\s/g, '').split('_')[0] + "_" + svcName;
            GetRTFResult(gdsJsonRTF, RtfFlightName);
            // RTF RESULT BIND -DEVESH
       ...
       // $('.curve').addClass('rtfpanel');
        ...
        setTimeout(function () {
            ...
            if ($.trim(id) == "DivNormalfare") {                
                ...
                RTFSH("1");
                RTFFirstTimeForSlider = 0;
                $('.SpecialRTF').show();
                $('#LoadRtFS').show();
                $('#divResult').html("");
                $('#divFrom').html("");
                // GetRTFResult(gdsJsonRTF, id);
                ShowSpecialRtfFare(gdsJsonRTF, id);
                $('#LoadRtFS').hide();
...
function getMinPriceRTF(ViewResult, airName) {
    ...
    //         .Where(function (item) { return item.fltName == airName.split('_')[1] + '_RTF'; })
    //         .Select(function (item) { return item.fltJson; });
    ...
    // .Select(function (item) { return item.TotalFare; });
    //airlineNameRTF1 = $(fltArray.items[0]).find('.ContentDiv')
    // parseFloat(parseFloat(item.AdtFare) * parseFloat(item.Adult) + parseFloat(item.ChdFare) * parseFloat(item.Child) + parseFloat(item.InfFare) * parseFloat(item.Infant)).toString()

    var arr = new Array();
    //for (var t = 0; t < airlineNameRTF1.length; t++) {
    //    if (($.trim($(airlineNameRTF1[t].innerHTML).find('.airlineImage').html()).replace(/\s/g, '').toLocaleLowerCase() == $.trim(airName.split('_')[0].toLocaleLowerCase()))) {
    //        arr.push({ id: t, val: $(airlineNameRTF1[t].innerHTML).find('.price').html() });
    ...
    //var minfareIndex = arr.indexOf(sorted[0]);
    //var maxfareIndex = arr.indexOf(sorted[sorted.length - 1]);

    // $('#MinFareRTF').html($($(airlineNameRTF1).find('.CheckSub')[minfareIndex]).val());

    //$('#MaxFareRTF').html($($(airlineNameRTF1).find('.CheckSub')[maxfareIndex]).val());


    //var minln = $($(airlineNameRTF1).find('.CheckSub')[minfareIndex]).val();
    //var maxln = $($(airlineNameRTF1).find('.CheckSub')[maxfareIndex]).val();
    //var keys = [];
    ...
    //        try {
    //            var keyID = keys[j];
    //            totData = totData + "<div  class='ContentDiv list-item'>" + airlineNameRTF1[keyID].innerHTML + "</div>";
    //        }
    //        catch (ex) {
    //        }
    //    }
    ...
    if (sorted.length > 0) {
        ...
        //airlineNameRTFAll.push({ id: airName.split('_')[1] + '_RTF', html: '', minprice: parseFloat(sorted[0]), maxprice: parseFloat(sorted[sorted.length - 1]), minLn: minLnNO.items[0], maxLn: maxLnNO.items[0] });
...
function NoResultMessage(st) {

   try {
    $('#fltgo').html("");
    $('#fltbk').html("");
    $('#totalPay').html("Your Selection");
    ...
    if ($('.hidtriptype').val().toLowerCase() == "r" && $('#FltSearch_hidFrom').val().split(',')[1] == "IN" && $('#FltSearch_hidTo').val().split(',')[1] == "IN") {
        ...
        if (($.trim($('.divFrom').html()) == "") && stage >= 14) {
            $('.divFrom').html(message);
        }

        if (($.trim($('.divTo').html()) == "") && stage >= 14) {
        ...
        if (($.trim($('.divResult').html()) == "") && stage == 7) {
    ...
    if ($.trim($('.divResult').html()).search("Sorry") > 0 && stage == 7 && st == 3) {
        $("#FilterBox").hide();
    }
    if ($.trim($('.divFrom').html()).search("Sorry") > 0 && stage >= 14 && st == 3) {
        $("#FilterBox").hide();
    }
    if ($.trim($('.divTo').html()).search("Sorry") > 0 && stage >= 14 && st == 3) {
        $("#FilterBox").hide();
    }
   }
    catch (error) {
...
function GetRTFResult(ViewResult, airName) {
    
    airlineNameRTF = airName.split('_')[0];
    var totData = "";

    var fltArray = JSLINQ(ViewResult)
            .Where(function (item) { return $.trim(item.fltName).replace(/\s/g, '') == airName.split('_')[0]; })
            .Select(function (item) { return item.fltJson; });
    
    var res = new Array();
    res.push({ fltsearch1: fltArray.items[0], fltName: airName.split('_')[0] });
    // getResultO(res[0]);
    var g = new ResHelper();    
    g.flterR.hide();
    g.OnewayH.show();
    g.flterRTF.show();
    //t.divFromResult.prepend(t.GetResultO(resultG1[0], maxLineNum, Airline + "ITZCACHE"));    
    //g.GetResultO(res[0].fltsearch1, fltArray.items[0][fltArray.items[0].length - 1].LineNumber, airName.split('_')[0] + "ITZCACHE")
    var res1 = new Array();
    res1.push( res[0].fltsearch1);  
    var RTFResultNew = g.GetResultO(res1, fltArray.items[0][fltArray.items[0].length - 1].LineNumber, 'RTFSPECIAL');
    //g.DivResult.html(RTFResultNew);

    $('#divResultRTF_' + airName).html(RTFResultNew);

    if (res1[0].length > 0) {
        
       // g.hdnOnewayOrRound.val("OneWay");
        //g.flterR.hide();
        //g.flterO.hide()
        //g.flterRTF.show();
        g.Book(res1);
        //if (SRTFFilter == true)
        //{
        //g.GetFltDetails(res1);// Coment By Devesh-25-04-2018
        //g.ShowFareBreakUp(res1);       // Coment By Devesh-25-04-2018
            SRTFFilter = false;
        //}        
        // t.DivMatrix.html(t.GetMatrix(CommonResultArray[0], 'O'));
        g.FltrSortRTF(res1, airName.split('_')[0]);
    }
    g.DivResult.show();
    $.unblockUI();
...
function ShowSpecialRtfFare(ViewResult, airName) {
    ...
    //    g.flterRTF.show();
    //    g.Book(res1);
    //    g.GetFltDetails(res1);
    //    g.ShowFareBreakUp(res1);        
    //    g.FltrSortRTF(res1);
    //}
    ...
    $('.OnewayReturn').hide();
    $('.listRTF').hide();
    $('#divResultRTF_' + airName).show();
    $('#divFilterRTF').show();

    $('.SRTF').hide();
    $('#FilterRTF' + airlineNameRTF).show();
    ...
    // retrieveDataFromResult($('.hidtriptype').val());
}


function WriteFilter(Aircode,AirName) {
    var res = '';

    res += '<div class="w95 auto SRTF" style="padding-top: 10px; display:none;" id="FilterRTF' + AirName + '" >';
    res += '<div class="large-12 medium-12 small-12 columns passenger"></div>';

        //Filter By Price
    res += '<div class="large-12 medium-12 small-12 columns" id="FltrPriceRTF' + Aircode + '">';
    res += '<div class="bld closeopen" onclick="fltrclick(this.id)" id="FBPRTF' + Aircode + '">Filter By Price</div>';
    res += '<div id="FBP1RTF' + Aircode + '" class="w100 lft " style="display: none;">';
    res += '<div class="clear2"></div>';
    res += '<div class="FoRTF">';
    res += '<div class="clsone">';
    res += '<div class="jplist-range-slider" data-control-type="range-sliderRTF' + Aircode + '" data-control-name="range-slider"';
    res += 'data-control-action="filter" data-path=".price">';
    res += '<div class="clear1"></div>';
    res += '<div class="ui-slider w90 mauto" data-type="ui-slider"></div>';
    res += '<div class="clear1"></div>';
    res += '<div class="lft w45">';
    res += '<span class="lft"><img src="../images/rs.png" style="height: 13px;" />&nbsp;</span>';
    res += '<span class="value lft" data-type="prev-value"></span>';
    res += '</div>';
    res += '<div class="rgt w45">';
    res += '<span class="value rgt" data-type="next-value"></span>';
    res += '<span class="rgt"><img src="../images/rs.png" style="height: 13px;" />&nbsp;</span>';
    res += '</div>';
    res += '</div>';
    res += '<div class="hidden" data-control-type="default-sort" data-control-name="sort" data-control-action="sort" data-path=".price" data-order="asc" data-type="number">';
    res += '</div>';
    res += '</div>';
    res += '</div>';
    ...
    res += '</div>';
    res += '</div>';
    res += '<div class="clear1">';
    res += '<hr />';
    res += '</div>';

    // Filter By Departure Time

    //res += '<div class="large-12 medium-12 small-12 columns" id="fltrTimeRTF' + Aircode + '">';
    //res += '<div class="bld closeopen" onclick="fltrclick(this.id)" id="FBDTRTF' + Aircode + '">Filter By Departure Time</div>';
    //res += '<div id="FBDT1' + Aircode + '" class="w100 lft" style="display: none;">';
    //res += '<div class="FoRTF">';
    //res += '<div class="jplist-group" data-control-type="DTimefilterRTF' + Aircode + '" data-control-action="filter" data-control-name="DTimefilterRTF' + Aircode + '" data-path=".dtime" data-logic="or">';
    //res += '<div class="clear"></div>';

    //res += '<div class="lft wss20 abc1 bdrs">';
    //res += '<input value="0_6" id="CheckboxT1RTF' + Aircode + '" type="checkbox" title="Early Morning" />';
    //res += '<label for="CheckboxT1"></label>';
    //res += '<span>00 - 06</span>';
    //res += '</div>';
    //res += '<div class="lft wss20 abc2t bdrs">';
    //res += '<input value="6_12" id="CheckboxT2RTF' + Aircode + '" type="checkbox" title="Morning" />';
    //res += '<label for="CheckboxT2"></label>';
    //res += '<span>06 - 12</span>';
    //res += '</div>';

    //res += '<div class="lft wss20 abc3t bdrs">';
    //res += '<input value="12_18" id="CheckboxT3RTF' + Aircode + '" type="checkbox" title="Mid Day" />';
    //res += '<label for="CheckboxT3"></label>';
    //res += '<span>12 - 18</span>';
    //res += '</div>';
    //res += '<div class="lft wss20 abc4t bdrs">';
    //res += '<input value="18_0" id="CheckboxT4RTF' + Aircode + '" type="checkbox" title="Evening" />';
    //res += '<label for="CheckboxT4"></label>';
    //res += '<span>18 - 00</span>';
    //res += '</div>';
    ...
    //Filter By Airline
    res += '<div class="large-12 medium-12 small-12 columns">';
    res += '<div class="bld closeopen" onclick="fltrclick(this.id)" id="FBARTF' + Aircode + '">Filter By Airline</div>';
    res += '<div class="w100 lft " id="FBA1RTF' + Aircode + '">';
    res += '<div class="clear"></div>';
    res += '<div id="airlineFilterRTF' + Aircode + '" class="fo" style="overflow-y: auto; max-height: 270px; overflow-x: hidden; white-space: nowrap;"></div>';
    res += '<div class="clear"></div>';
    res += '</div>';
    ...
    res += '</div>';
        ...
        //Filter By Stops
    res += '<div class="large-12 medium-12 small-12 columns">';
    res += '<div class="bld closeopen" onclick="fltrclick(this.id)" id="FBSRTF' + Aircode + '">Filter By Stops</div>';
    res += '<div class="w100 lft" id="FBS1RTF' + Aircode + '">';
    res += '<div class="clear"></div>';
    res += '<div id="stopFlterRTF' + Aircode + '" class="FoRTF" style="overflow-y: auto; max-height: 170px; overflow-x: hidden; white-space: nowrap;"></div>';
    res += '</div>';
    res += '</div>';
    ...
    res += '</div>';
        ...
        //Filter By Fare Rule
    res += '<div class="large-12 medium-12 small-12 columns">';
    res += '<div class="bld closeopen" onclick="fltrclick(this.id)" id="FBFTRTF' + Aircode + '">Filter By Fare Rule</div>';

    res += '<div class="w100 lft" id="FBFT1RTF' + Aircode + '" style="display: none;">';
    res += '<div class="FoRTF">';
    res += '<div class="jplist-group" data-control-type="RfndfilterRTF' + Aircode + '" data-control-action="filter" data-control-name="RfndfilteRTF' + Aircode + '" data-path=".rfnd" data-logic="or">';
    res += '<div class="clear"></div>';
    res += '<div class="lft w8">';
    res += '<input value="r" id="CheckboxR1RTF' + Aircode + '" type="checkbox" />';
    res += '</div>';
    res += '<div class="lft w80" style="padding-top: 3px">';
    res += '<label for="CheckboxR1">Refundable</label>';
    res += '</div>';
    res += '<div class="clear"></div>';
    res += '<div class="lft w8">';
    res += '<input value="n" id="CheckboxR2RTF' + Aircode + '" type="checkbox" />';
    res += '</div>';
    res += '<div class="lft w80" style="padding-top: 3px">';
    res += '<label for="CheckboxR2">Non Refundable</label>';
    res += '</div>';
    res += '</div>';
    res += '</div>';
    res += '</div>';
    ...
    res += '</div>';
        ...
        //Airline Fare Type
    res += '<div class="large-12 medium-12 small-12 columns">';
    res += '<div class="bld closeopen" onclick="fltrclick(this.id)" id="DAFTRTF' + Aircode + '">Airline Fare Type</div>';
    res += '<div class="w100 lft " id="DAFT1RTF' + Aircode + '">';
    res += '<div class="clear"></div>';
    res += '<div id="AirlineFareTypeRTF' + Aircode + '" class="FareTypeRTF FoRTF" style="overflow-y: auto; max-height: 270px; overflow-x: hidden; white-space: nowrap;"></div>';
    res += '<div class="clear1">';
    res += '<hr />';
    res += '</div>';
