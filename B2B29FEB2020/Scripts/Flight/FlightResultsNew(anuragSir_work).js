...
ResHelper.prototype.GetSearchDisplay = function (c, triptype) {
    ...
    if (triptype == "rdbOneWay") {
        ...
        st += '  <div class="lft ">';
        st += t.hidtxtDepCity1.val().split(',')[1] + '<br />';
...
ResHelper.prototype.GetFltDetailsR = function (result) {
    ...
    $('.fltDetailslinkR').click(function (event) {
           ...
           .Select(function (item) { return item });
            ...
            if (O.items.length > 0) {
                ...
                for (var i = 0; i < O.items.length; i++) {
                    ...
                    if (O.items[0].Trip == "I") {
                        ftme = O.items[i].TripCnt + ' HRS<br/>';
                    }
                    //else if (O.items[0].Trip == "I") {
                    //    ftme = "&nbsp;";
                    //}
                    str1 += '<div class="large-2 medium-2 small-2 columns bld">' + ftme + '</div>';
                    str1 += '<div class="large-3 medium-3 small-3 columns"><span>' + O.items[i].DepartureLocation + '&nbsp;' + [O.items[i].DepartureTime.replace(":", "").slice(0, 2), ":", O.items[i].DepartureTime.replace(":", "").slice(2)].join('') + '</span><br />' + O.items[i].DepartureCityName + '<br />' + O.items[i].Departure_Date + '<br />Terminal - ' + O.items[i].DepartureTerminal + '</div>';
                    str1 += '<div class="large-2 medium-2 small-2 columns dvsrc"></div>';
                    str1 += '<div class="large-3 medium-3 small-3 columns"><span>' + [O.items[i].ArrivalTime.replace(":", "").slice(0, 2), ":", O.items[i].ArrivalTime.replace(":", "").slice(2)].join('') + '&nbsp;' + O.items[i].ArrivalLocation + '</span><br />' + O.items[i].ArrivalCityName + '<br />' + O.items[i].Arrival_Date + '<br />Terminal - ' + O.items[i].ArrivalTerminal + '</div><div class="clear"></div></div>';
        ...
        if (lineNumshd[1] == "O") {
            ...
            $('#' + lineNumshd[0] + '_R').html(str1);
            //$('#' + this.rel + '_').show();
            $('#' + lineNumshd[0] + '_R').slideToggle();
        }
        //$('#' + this).hide();
    ...
    $('.fltBagDetailsR').click(function (event) {
        ...
        if (lineNumsbghd[1] == "R") {
            $('#' + lineNumsbghd[0] + '_R').html(str1);
            //$('#' + this.rel + '_').show();
            $('#' + lineNumsbghd[0] + '_R').slideToggle();
        }
        //$('#' + idr).html(str1);
        //$('#' + idr).fadeToggle(1000);
    });
        ...
        function doChunkO() {
            ...
            for (; i <= LnOb; i++) {
                ...
                if (OB.items.length > 0) {
                    ...
                    result += ' </div>';
                    result += '<div class="depcity" id="' + OB.items[obi].LineNumber.toString() + '_" style="display:none;margin-top:-11px;"></div>';
                    result += ' </div>';
                }
                ...
                for (; a <= LnIb; a++) {
                    ...
                    if (OR.items.length > 0) {
                        ...
                        for (var obr = 0; obr < 1; obr++) { //OR.items.length; obr++) {
                            ...
                            result1 += '<div class="clear"></div>';
                            result += '<div class="depcity" id="' + OR.items[obr].LineNumber.toString() + '_R" style="display:none;margin-top:-11px;"></div>';
                        }
               ...
               .Select(function (item) { return item });
                ...
                for (var ff = 2; ff <= ft; ff++) {
                    ...
                    if (RF.items.length > 0) {
                        ...
                        result += '</div>';
                        result += '<div class="row">';
                        result += '<div><span>' + RF.items[0].DepartureCityName + '</span> <i class="fa fa-long-arrow-right"></i> <span>' + RF.items[RF.items.length - 1].ArrivalCityName + '</span></div>'
                        result += '</div>';
...
function DisplayMultipleFares(list, lnNoArray, secNum, fltName, IsRTrip, IsRTF, Index, tripF, mainLineNo) {
    ...
    if (IsRTrip == true) {
        ...
        if (tripF == "O") {
            res += '<div class="col-md-2" style="z-index: 800;width: 30%;left: -44px;font-weight: 600;">';
        }
        else { res += '<div class="col-md-2" style="z-index: 800;width: 30%;left: -12px;font-weight: 600;">'; }
    }
    ...
    for (var i = 0; i < lnn.length; i++) {
        ...
        if (IsRTrip == true) {
            if (tripF == "O") {
                ...
                res += '<label class="radio inline" style="font-size: 11px;font-weight: 700;"><input type="radio" name="O" class="' + mainLineNo + '" value="' + OF.items[0].LineNumber + '" style="position: relative;"/>' + OF.items[0].TotalFare + '&nbsp&nbsp&nbsp&nbsp' + OF.items[0].DisplayFareType + ' </label>';
            }
            else if (tripF == "R") {
                res += '<label class="radio inline" style="font-size: 11px;font-weight: 700;"><input type="radio" name="R" value="' + OF.items[0].LineNumber + '" style="position: relative;"/>' + OF.items[0].TotalFare + '&nbsp&nbsp&nbsp&nbsp' + OF.items[0].DisplayFareType + '</label>';
            }
            //else {
    ...
    for (var i = 0; i < lnn.length; i++) {
        ...
        if (IsRTrip == true) {
            ...
            if (tripF == "O") {
                ...
                if (i == 0) {
                    faredet += '<div value="' + mainLineNo + '" class="' + mainLineNo + '_faredetailmasterall" id="' + OF.items[0].LineNumber + '_faredetailmaster" style="position: relative; top: 17px; float: left;right: 82%; width:460px;">';
                ...
                faredet += '<div value="' + mainLineNo + '" class="' + mainLineNo + '_faredetailmasterall" id="' + OF.items[0].LineNumber + '_master" style="display:none;position: relative; top: 17px; float: left;right: 82%;width:460px;">' + rnr;
                ...
                if (i == 0) {
                    faredet += '<div value="' + mainLineNo + '" class="' + mainLineNo + '_faredetailmasterall" id="' + OF.items[0].LineNumber + '_faredetailmaster_inb" style="position: relative; top: 22px; float: left;right: 338px; width:465px;">';
                ...
                faredet += '<div value="' + mainLineNo + '" class="' + mainLineNo + '_faredetailmasterall" id="' + OF.items[0].LineNumber + '_master_inb" style="display:none;position: relative; top: 22px; float: left;right: 338px; width:465px;">' + rnr;
                //faredet += '<div class="gridViewToolTip cursorpointer lft"><span style="font-weight:600;color:orange !important;">Fare Details  |  </span></div>';
                //faredet += '<div class="lft div_cls"><a href="#" id="' + OF.items[0].LineNumber + '_flt" title="' + mainLineNo + '" rel="' + mainLineNo + '"  class="gridViewToolTip"><span style="font-weight:400;color:orange !important;">Fare Details</span></a>&nbsp;&nbsp;<span>|</span>&nbsp;&nbsp;</div>';
                //faredet += '<div class="lft div_cls"><a href="#" id="' + OF.items[0].LineNumber + 'flt" title="' + mainLineNo + '" rel="' + mainLineNo + '"  class="gridViewToolTip"><span style="font-weight:400;color:orange !important;">Fare Details</span></a>&nbsp;&nbsp;<span>|</span>&nbsp;&nbsp;</div>';
                faredet += '<div class="gridViewToolTip1 lft"  title="' + OF.items[0].LineNumber + '_O" ></div>';
