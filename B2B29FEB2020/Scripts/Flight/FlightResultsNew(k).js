        ...
        if (Trip == "D") {
            ...
            try {
                $.ajax({
                    ...
                    data: "{ 'name': 'FlightDom/FltResult.aspx'}",
                    dataType: "json",
                    type: "POST",
            ...
            try {
                $.ajax({
                    url: UrlBase + "FltSearch1.asmx/GetMUForPage",
                    data: "{ 'name': 'FlightInt/FltResIntl.aspx'}",
                    dataType: "json",
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
            ...
            $.ajax({
                ...
                success: function (e) {
                    ...
                    if (e.d.ChangeFareO.TrackId == "0") {
                        alert("Selected fare has been changed.Please select another flight1.");
                        // $("#searchquery").show();
                        // $(document).ajaxStop($.unblockUI);
                        ...
                        window.location = UrlBase + "FlightInt/CustomerInfo.aspx?" + e.d.ChangeFareO.TrackId + ",I";
                    }
                },
                error: function (e, t, n) {
            ...
            $.ajax({
                ...
                success: function (e) {
                    ...
                    if (e.d.ChangeFareO.TrackId == "0") {
                        // alert("Selected fare has been changed.Please select another flight.");
                        // window.location = UrlBase + "IBEHome.aspx";
                        var r = confirm("Selected fare has been changed.Please select another flight,search again!");
                        if (r == true) {
                            location.reload();
                        }
                        $('#ConfmingFlight').hide();
...
ResHelper.prototype.GetUniqueAirline = function (result, cls, type) {
    ...
    var OAC = JSLINQ(result)
       .OrderBy(function (item) { return item.ValiDatingCarrier })
    ...
    for (var i = 0; i < marray.length; i++) {
        ...
        str += '<div class="lft w8"> <input value="' + marray[i] + '"  id="CheckboxA' + type + i + 1 + '"  type="checkbox"  /> </div><div class="lft w80" style="padding-top:0px;"><label for="' + marray[i] + '"><span><img alt="" src="../Airlogo/sm' + marrayalcode[i] + '.gif" style="width:25px;"><span>  ' + marray[i] + '</label> </div> <div class="clear"> </div>';
            ...
            if (O.items.length > 0) {
                ...
                for (var i = 0; i < O.items.length; i++) {
                    ...
                    if ((O.items[i].MarketingCarrier == '6E') && ($.trim(O.items[i].sno).search("INDIGOCORP") >= 0)) {
                        str1 += '<div class="large-2 medium-2 smallf20 rgt colorp-2 columns"><img alt="" src="' + UrlBase + 'AirLogo/smITZ.gif" /><br />' + O.items[i].FlightIdentification + '<br/>Class(' + O.items[i].AdtRbd + ')</div>';
                    }
                    else {
            ...
            for (; i <= LnOb; i++) {
                ...
                if (OB.items.length > 0) {
                    ...
                    for (var obi = 0; obi < 1; obi++) { //OB.items.length; obi++) {
                        ...
                        result += '</div>';
                        result += '<div class="f16"> ' + t.MakeupAdTime(OB.items[OB.items.length - 1].ArrivalTime) + '</div>';
                        ...
                        if (k == 0) {
							...
							//RoundTrip LEFT
                            result += '<div class="spnDiscountShowHide large-6 medium-6 small-6 columns" style="display:none;">Inv Price.' + OB.items[obi].NetFare + '&nbsp;Incentive : ' + OB.items[obi].TotDis + '</div>';
                        }
                        result += '</div>';
                        result += '</div>';
                        if (k == 0) {
        ...
        for (i = 1; i <= LnOb; i++) {
            ...
            if (OB.items.length > 0) {
               ...
               .Select(function (item) { return item });
                ...
                /////Open div
                result += '<div class="fltbox mrgbtm" style="border: 1px solid #dad8d8;box-shadow: 0 2px 0 0px rgba(0,0,0,.08);-webkit-box-shadow: 0 2px 0 0px rgba(0,0,0,.08);-moz-box-shadow: 0 2px 0 0px rgba(0,0,0,.08);border-radius: 4px;">';
                ///
                //}
                ...
                if (OF.items.length > 0) {
                    ...
                    result += '<div class="large-2 medium-1 small-3 columns passenger" style="margin-top: 27px;margin-left: -42px;">';
                    ...
                    result += '<div style="float: left;text-align:center;font-size: 13px;color: #000;">' + totDur + ' |  <span style="font-size:13px;">' + OF.items[0].Stops + '</span></div>'
                    result += '</div>'
                    result += '<div class="row">'
                    result += '<div><span>' + OF.items[OF.items.length - 1].DepartureLocation + '</span> <i class="fa fa-long-arrow-right"></i> <span>' + OF.items[OF.items.length - 1].ArrivalLocation + '</span></div>'
                    result += '</div>'
                   ...
                   // if (OB.items[0].ValiDatingCarrier != 'SG' && OB.items[0].ValiDatingCarrier != '6E' && OB.items[0].ValiDatingCarrier != 'G8')
                    ...
                    result += '<div class="col-md-5" style="margin-left: -10px;">';
                    result += '<div class="row">'
                    ///result += '';
                    if ($.trim(Provider).search("SFM") > 0) {
                   ...
                   // result += '<div class="w100 rgt"><input type="button"  value="Book_Dev2"  class="buttonfltbk" title="' + OB.items[0].LineNumber + '"  id="' + OB.items[0].LineNumber + '" /></div>';
                    ...
                    //Use Price Fare wise
                    result += '<div class="col-md-8" style="width: 262px;">';
                ...
                //////div1 close
                result += '<div id="' + OB.items[0].LineNumber.toString() + '_" style="display:n o ne;width:100%;"></div>';
                //result += '<div class="clear"></div>';
                    ...
                    if (Obook[0].AirLineName == Rbook[0].AirLineName && ObookSFM != null && RbookSFM != null && ObookSFM.length != 0 && RbookSFM.length != 0 && Obook[0].AdtFar == "NRM" && Rbook[0].AdtFar == "NRM" && rtfBoolStatus == true) {
                        ...
                        if (ObookSFM[0].ValiDatingCarrier.toUpperCase() == "6E" && RbookSFM[0].ValiDatingCarrier.toUpperCase() == "6E") {
                            ...
                            if (SRFResultAfterReprice != null && SRFResultAfterReprice.length > 0 && SRFResultAfterReprice[0].d.length > 0 && SRFResultAfterReprice[0].d[0].TotalFare > 0) {
                                //if 2
                                SRFReprice = true
                                e.RtfTotalPayDiv.html('<input type="radio" id="radio1" name="RadioGroup1" value="1" checked="true"/>Normal Fare: <img src="' + UrlBase + 'images/crs.png" /> ' + parseInt(Obook[0].TotalFare + Rbook[0].TotalFare) + '/-  <input type="radio" id="radio2" name="RadioGroup1" value="2" />SRF Fare: <img src="' + UrlBase + 'images/crs.png" /> ' + SRFResultAfterReprice[0].d[0].TotalFare + '/-');
                                ...
                                //if else 2
                                e.RtfTotalPayDiv.html('<input type="radio" id="radio1" name="RadioGroup1" value="1" checked="true"/>Normal Fare: <img src="' + UrlBase + 'images/crs.png" /> ' + parseInt(Obook[0].TotalFare + Rbook[0].TotalFare) + '/-');
                                // totcurrentFareSFM = parseInt(ObookSFM[0].TotalFare + RbookSFM[0].TotalFare);
                            ...
                            //if else 1
                            e.RtfTotalPayDiv.html('<input type="radio" id="radio1" name="RadioGroup1" value="1" checked="true"/>Normal Fare: <img src="' + UrlBase + 'images/crs.png" /> ' + parseInt(Obook[0].TotalFare + Rbook[0].TotalFare) + '/-  <input type="radio" id="radio2" name="RadioGroup1" value="2" />SRF Fare: <img src="' + UrlBase + 'images/crs.png" /> ' + Math.ceil(ObookSFM[0].TotalFare + RbookSFM[0].TotalFare) + '/-');
                            totcurrentFareSFM = parseInt(ObookSFM[0].TotalFare + RbookSFM[0].TotalFare);
                        }
                        //						
                    }
                    else {
                        e.RtfTotalPayDiv.html('Current Fare- <i class="fa fa-inr" aria-hidden="true"></i> ' + parseInt(Obook[0].TotalFare + Rbook[0].TotalFare) + '/-');
                        totcurrentFare = parseInt(Obook[0].TotalFare + Rbook[0].TotalFare);
                    }
                    ...
                    if (olnum.search('SFM') > 0 && rlnum.search('SFM') > 0) {
                        if (ovc.toLowerCase() != rvc.toLowerCase()) {
                         ...
                         .Select(function (item) { return item });
                                ...
                                e.RtfTotalPayDiv.html('Current Fare- <i class="fa fa-inr" aria-hidden="true"></i> ' + parseInt(Obook[0].TotalFare + Rbook[0].TotalFare) + '/-<div class="clear1"></div>');
                                e.RtfTotalPayDiv.css("display", "block");
                                e.RtfBookBtn.css("display", "block");
                                isSRF = true;
                                totcurrentFare = parseInt(Obook[0].TotalFare + Rbook[0].TotalFare);
...
ResHelper.prototype.DisplaySelectedFlight = function (objFlt, type) {
    ...
    if (objFlt.length > 1) {
        ...
        string += '<div class="large-6 medium-6 small-6 columns"><div>' + objFlt[0].DepartureCityName + ' - ' + objFlt[objFlt.length - 1].ArrivalCityName + '</div><div class="passenger lft ">' + objFlt[0].DepartureTime + ' - ' + objFlt[objFlt.length - 1].ArrivalTime + '</div><div class="passenger lft f12" style="padding:2px 10px;font-weight:bold">' + '&nbsp;&nbsp;' + objFlt[0].Departure_Date + '</div></div>';
        if (type == "SRF") {
        ...
        string += '<div class="large-6 medium-6 small-6 columns"><div>' + objFlt[0].DepartureCityName + ' - ' + objFlt[0].ArrivalCityName + '</div><div class="passenger lft ">' + objFlt[0].DepartureTime + ' - ' + objFlt[0].ArrivalTime + '</div><div class="passenger f12 bld" style="padding:2px 10px;font-weight:bold">' + '&nbsp;&nbsp;' + objFlt[0].Departure_Date + '</div></div>';
...
ResHelper.prototype.GetUniqueAirlineFareType = function (result, cls, type) {
    ...
    for (var i = 0; i < marray.length; i++) {
        ...
        if (FareTypeS1 != "") {
            ...
            //SpecialFare = false;
            //NormalFareF == false;
            str += '<div class="lft w8"> <input value="' + FareTypeS1 + '"  id="CheckboxA' + type + i + 1 + '"  type="checkbox"  /> </div><div class="lft w80" style="padding-top:0px;"><label for="' + FareTypeS1 + '">' + FilterFareTypeShow + '</label> </div> <div class="clear"> </div>';
...
function displayBIn(id, d) {
   ...
   .Select(function (item) { return item });
    ...
    var str1 = '<div>';
    if (O.items.length > 0) {
        str1 += '<div class="depcity"><span style="font-size:20px;display:none; float:right; position:relative; top:-5px; right:-15px; cursor:pointer; height:1px;" onclick="Close(\'' + this.rel + '_\');" title="Click to close Details">X</span><div></div>';
        str1 += '<table class="w100 f12"><tr><td  class="w50 f16 bld">Sector</td><td class="f16 bld">Baggage Quantity</td></tr>';
        for (var i = 0; i < O.items.length; i++) {
            str1 += '<tr><td>' + O.items[i].DepartureCityName + '-' + O.items[i].ArrivalCityName + '(' + O.items[i].MarketingCarrier + ' - ' + O.items[i].FlightIdentification + ')</td>'
...
function DisplayMultipleFares(list, lnNoArray, secNum, fltName, IsRTrip, IsRTF, Index, tripF, mainLineNo) {
    ...
    for (var i = 0; i < 1; i++) {
        ...
        if (IsRTrip == true) {
            if (tripF == "O") {
                res += '<input type="radio" name="O" value="' + OF.items[0].LineNumber + '" />' + OF.items[0].TotalFare + '&nbsp&nbsp&nbsp&nbsp' + OF.items[0].AdtFar;
            }
            else if (tripF == "R") {
                res += '<input type="radio" name="R" value="' + OF.items[0].LineNumber + '" />' + OF.items[0].TotalFare + '&nbsp&nbsp&nbsp&nbsp' + OF.items[0].AdtFar;
            }
            else {
                res += '<input type="radio" name="O" value="' + OF.items[0].LineNumber + '" />' + OF.items[0].TotalFare + '&nbsp&nbsp&nbsp&nbsp' + OF.items[0].AdtFar;
            }
            ...
            // New Line Add 12-02-2020
            //res += '<div class="w100 rgt"><input type="button"  value="Book_Dev2"  class="buttonfltbk" title="' + OB.items[0].LineNumber + '"  id="' + OB.items[0].LineNumber + '" /></div>';
            //res += '<div class="w100 rgt"><input type="radio" name="radioPrice" title="' + mainLineNo + '" id="' + OF.items[0].LineNumber + '_radio" value="' + OF.items[0].LineNumber + '" >' + OF.items[0].TotalFare + '&nbsp&nbsp&nbsp&nbsp' + OF.items[0].AdtFar + '</div>';
            //if (i == 0) {                
            //    res += '<div class="w100 rgt"><input type="button"  value="Book" class="falsebookbutton ' + mainLineNo + '" id="' + mainLineNo + '_falsebookbutton" /></div>';                
            //}           
            //res += '<div class="w100 rgt"><input type="button"  value="Book_Dev2"  class="buttonfltbk" style="display: none;" title="' + OF.items[0].LineNumber + '"  id="' + OF.items[0].LineNumber + '" /></div>';
            //if (i == 0) {
            //    res += '<div class="col-md-2"><input type="button"  value="Book" class="falsebookbutton ' + mainLineNo + '" id="' + mainLineNo + '_falsebookbutton" style="bottom: 3px;position: relative;margin-left: 56px"/></div>';
            //}
            //res += '<div class="col-md-2"><input type="button"  value="Book_Dev2"  class="buttonfltbk" style="display: none;" title="' + OF.items[0].LineNumber + '"  id="' + OF.items[0].LineNumber + '" style="bottom: 3px;position: relative;margin-left: 56px"/></div>';
            //End - New Line Add 12-02-2020
            ...
            //res += '<div>' + rnr;            
            //res += '<div class="gridViewToolTip cursorpointer lft"><img src="' + UrlBase + 'images/icons/faredetails.png"   />&nbsp;</div>';           
            //res += '<div class="gridViewToolTip1 hide"  title="' + OF.items[0].LineNumber + '_O" >ss</div>';
            //res += '<div class="lft"><a href="#" id="' + OF.items[0].LineNumber + 'Det" title="' + mainLineNo + '" rel="' + OF.items[0].LineNumber + '"  class="fltDetailslink"><img src="' + UrlBase + 'images/icons/information.png" class="cursorpointer" alt="Flight Details" title="Flight Details"   /></a></div>';
            //res += '<div class="lft">&nbsp;<a href="#" id="' + OF.items[0].LineNumber + 'BagDet"  title="' + mainLineNo + '" rel="' + OF.items[0].LineNumber + '"  class="fltBagDetails"> <img src="../images/icons/baggage.png"   alt="Baggages Allowed"  class="cursorpointer"title="Baggages Allowed"></a></div>';            
            //res += '<span class="fareRuleToolTip cursorpointer lft" rel="FareRule_' + OF.items[0].LineNumber + '_O" title="' + mainLineNo + '" ><img src="' + UrlBase + 'images/fare-rules.png"  class="cursorpointer" alt="Fare Rule" title="Fare Rule" style="cursor:pointer;" /> &nbsp;</span><div class="fade"  title="' + OF.items[0].LineNumber + '_O" > &nbsp;</div>';            
            //res += '</div>';
            ...
            //res += ''
            res += '<div style="width: 100%;font-size:11px;"><div class="w100 rgt"><input type="radio" name="Price" value="' + OF.items[0].LineNumber + '" ><span style="font-size:13px;font-weight:600;color:#000;"> ₹ ' + OF.items[0].TotalFare + '</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span></span><span class="spnDiscountShowHide" style="display:none;font-size: 11px;font-weight:400;color:#969696;">|  Net : ₹ ' + OF.items[0].NetFare + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span> | </span> Inv :' + OF.items[0].TotDis + ' <span></div>';
            res += '<div class="row" style="position: inherit;margin-left: 1px;"><span style="font-size:10px;font-weight:900;color: #00a1ff;">CORPORATE</span><span> | </span><span style="font-size:12px;font-weight:400;color:#969696;">' + OF.items[0].AdtFareType + '</span> |  <i><img src="../Images/icons/airline-seat.png" style="width: 12px;margin-top: -5px;;"/></i><span style="font-size:12px;color:orange;font-weight:400;"> ' + OF.items[0].AvailableSeats1 + ' left</span> &nbsp;&nbsp;<a><i><img src="../Images/icons/12-200.png" style="width:12px;    margin-top: -5px;"/></i></a></div></div>'
            res += '<hr style="width: 265px;margin-left:0px;border-bottom: dotted #848484;position: absolute;"><i><img src="../Images/icons/img_259251.png" style="width: 16px;margin-left: 254px;margin-top: -2px;"/></i>'
            res += '<div style="width: 100%;font-size:11px;"><div class="w100 rgt"><input type="radio" name="Price" value="' + OF.items[0].LineNumber + '" ><span style="font-size:13px;font-weight:600;color:#000;"> ₹ ' + OF.items[0].TotalFare + '</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span></span><span class="spnDiscountShowHide" style="display:none;font-size: 11px;font-weight:400;color:#969696;">|  Net : ₹ ' + OF.items[0].NetFare + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span> | </span> Inv :' + OF.items[0].TotDis + ' </span></div>';
            res += '<div class="row" style="position: inherit;margin-left: 1px;"><span style="font-size:10px;font-weight:900;color: #26bf00;">COUPON</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span> | </span><span style="font-size:12px;font-weight:400;color:#969696;">' + OF.items[0].AdtFareType + '</span> |  <i><img src="../Images/icons/airline-seat.png" style="width: 12px;margin-top: -5px;"/></i><span style="font-size:12px;color:orange;font-weight:400;"> ' + OF.items[0].AvailableSeats1 + ' left</span> &nbsp;&nbsp;<a><i><img src="../Images/icons/12-200.png" style="width:12px;    margin-top: -5px;"/></i></a> </div></div>'
            res += '<hr style="width: 265px;margin-left:0px;border-bottom: dotted #848484;position: absolute;"><i><img src="../Images/icons/img_259251.png" style="width: 16px;margin-left: 254px;margin-top: -2px;"/></i>'
