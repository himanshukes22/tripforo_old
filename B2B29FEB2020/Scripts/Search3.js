...
$(document).ready(function () {
    ...
    if (e == "rdbOneWay") {
        //$("#trRetDateRow").hide()
        $("#txtRetDate").prop("disabled", true).css("color", "#eee").css("cursor", "no-drop").css("background", "none");
        MultiCityShowHide();
    }
    else if (e == "rdbRoundTrip") {
        $("#txtRetDate").prop("disabled", false).removeAttr("style");
        $("#trRetDateRow").show();
        MultiCityShowHide();
    }
    else if (e == "fixed") {
...
SearchHelper.prototype.BindEvents = function () {
            ...
            $.ajax({
                ...
                success: function (e) {
                    t($.map(e.d, function (e) {
                        var cityname = e.CityName.split(',')[0];
                        var airportname = e.CityName.split(',')[1];

                        var tval = e.CityName + "(" + e.AirportCode + ")";
                        var tlabel = cityname + " (" + e.AirportCode + ")";
    ...
    //        .append(item.label)
    //        .append("<br/><span><i class='icofont-location-pin' style='color:rgb(238 49 87);'></i> " + item.kunal + "</span>")
    //        .appendTo(ul);
    //};
    ...
    e.TripType.click(function () {
        ...
        if ($(this).val() == "rdbOneWay" || $(this).val() == "rdbRoundTrip" || $(this).val() == "rdbMultiCity") {
            ...
            if ($(this).val() == "rdbRoundTrip") {
                ...
                //$("#txtRetDate").removeAttr('disabled', 'disabled');
                $("#txtRetDate").prop("disabled", false).removeAttr("style");
                ...
                //$("#txtRetDate").prop("disabled", true).css("background", "#ccc").css("cursor", "no-drop");
                $("#txtRetDate").prop("disabled", true).css("color", "#eee").css("cursor", "no-drop").css("background", "none");
