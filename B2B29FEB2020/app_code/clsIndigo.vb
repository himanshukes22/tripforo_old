...
Imports System.Text
Public Class clsIndigo
    Dim objDB As New clsDbIndigo
    Dim TrackId As String = GetTraceIDForIndigo()
    ...
    Public Function getAvailabilty(ByVal Trip As String, ByVal TourType As String, ByVal sOrigin As String, ByVal sDestination As String, ByVal sDateOfDep As String, _
        ...
        sSearch = GetSearchCriteria(sClientID, sAPISource, sDistNo, sOrigin, sDestination, SetSearchDate(sDateOfDep, Trip), SetSearchDate(sReturnDate, Trip), _
        ...
        If dtTemp.Rows.Count > 0 Then
            dtFareQuote = GetFareQuoteDT(dtTemp, sClientID, sCarrierAccount, iTotalNo_Adt, iTotalNo_Chd, iTotalNo_Inf, iFareNo, Inf_Basic, Inf_Tax, sOrigin.ToUpper)
    ...
    End Function
    Private Function SetSearchDate(ByVal travelDate As String, ByVal trip As String) As String      
        Try
            If Convert.ToDateTime(travelDate) = Now.Date Then
                Dim hr As String = "00"
                Dim mm As String = "00"
                Dim chktime As Integer = 0
                If trip = "D" Then chktime = 22 Else chktime = 20
                If Now.Hour >= chktime And Now.Minute >= 0 Then
                    hr = "23"
                    mm = "59"
                Else
                    If trip = "D" Then
                        If (Now.Hour + 2).ToString.Length > 1 Then hr = (Now.Hour + 2).ToString Else hr = "0" & (Now.Hour + 2).ToString
                    Else
                        If (Now.Hour + 4).ToString.Length > 1 Then hr = (Now.Hour + 4).ToString Else hr = "0" & (Now.Hour + 4).ToString
                    End If
                    If Now.Minute.ToString.Length > 1 Then mm = Now.Minute.ToString Else mm = "0" & Now.Minute.ToString
                End If
                travelDate = travelDate & "T" & hr & ":" & mm & ":00Z"
            Else
                travelDate = travelDate & "T00:00:00Z"
            End If
    ...
    Private Function GetSearchCriteria(ByVal sClientID As String, ByVal sAPISource As String, ByVal sDistNo As String, _
        ...
        sXML += "<DepartureDate xsi:type='xsd:dateTime'>" & sDateOfDep & "</DepartureDate>" 'T00:00:00Z
        ...
        If sReturnDate <> "" Then
            sXML += "<item xsi:type='ns1:AvailabilityQuery'>"
            sXML += "<DepartureDate xsi:type='xsd:dateTime'>" & sReturnDate & "</DepartureDate>" 'T00:00:00Z
    ...
    Private Function GetFareQuoteDT(ByVal dtTable As DataTable, ByVal sClientID As String, ByVal sCarrierAccount As String, ByVal iTotalNo_Adt As Integer, ByVal iTotalNo_Chd As Integer, ByVal iTotalNo_Inf As Integer, ByVal iFareNo As Integer, ByVal Inf_Basic As Double, ByVal Inf_Tax As Double, ByVal Origin As String) As DataTable
        Dim dtTemp As New DataTable
        ...
        Try
            Do While iData <= dtTable.Rows.Count - 1
                ...
                'sResponse = PostXml("http://directnettest.navitaire.com:8090/servlet/rpcrouter", sRequest)
                            ...
                            Do While iCtr <= xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.Count - 1
                                If sPassTypeCode = "ADT" Then
                                    ...
                                    ElseIf xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).LocalName = "Taxes" Then
                                        Dim tax2 As Double = 0
                                        Dim tax3 As Double = 0
                                        Dim tax4 As Double = 0
                                        Dim tax5 As Double = 0
                                        Dim bf As Double = 0
                                        For i3 As Integer = 0 To xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.Count - 1
                                            'If TourType.ToUpper <> "RETURN" Then
                                            ...
                                            'Else
                                            If Origin <> "SIN" And Origin <> "KTM" And Origin <> "MCT" And Origin <> "BKK" And Origin <> "DXB" And Origin <> "IXA" And Origin <> "IMF" And Origin <> "SXR" And Origin <> "IXJ" And Origin <> "DIB" And Origin <> "GAU" Then
                                                If xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).LastChild.InnerText = "2" Then
                                                    If tax2 = 0 Then
                                                        tax2 = Val(xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).ChildNodes.ItemOf(1).ChildNodes.ItemOf(0).InnerText)
                                                    End If
                                                ElseIf xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).LastChild.InnerText = "3" Then
                                                    If tax3 = 0 Then
                                                        tax3 = Val(xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).ChildNodes.ItemOf(1).ChildNodes.ItemOf(0).InnerText)
                                                    End If
                                                ElseIf xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).LastChild.InnerText = "4" Then

                                                ElseIf xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).LastChild.InnerText = "5" Then
                                                    If tax5 = 0 Then
                                                        tax5 = Val(xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).ChildNodes.ItemOf(1).ChildNodes.ItemOf(0).InnerText)
                                                    End If
                                                Else
                                                    dTaxAmount_Adt = dTaxAmount_Adt + xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).ChildNodes.ItemOf(1).ChildNodes.ItemOf(0).InnerText
                                                End If
                                                bf = (Val(row("BaseAmt_Adt")) * 4.95) / 100
                                            Else
                                                dTaxAmount_Adt = dTaxAmount_Adt + xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).ChildNodes.ItemOf(1).ChildNodes.ItemOf(0).InnerText
                                            End If
                                            'End If
                                        Next
                                        tax4 = Math.Round(((tax2 * 4.95) / 100), 0) + Math.Round(((tax3 * 4.95) / 100), 0) + Math.Round(((tax5 * 4.95) / 100), 0) + Math.Round(bf, 0)
                                        row("TaxAmt_Adt") = dTaxAmount_Adt + tax2 + tax3 + tax4 + tax5
                                        row("TotalAmt_Adt") = (Val(row("TaxAmt_Adt")) + Val(row("BaseAmt_Adt"))).ToString
                                        ...
                                        ElseIf xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).LocalName = "Taxes" Then
                                            ...
                                            For i3 As Integer = 0 To xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.Count - 1
                                                If Origin <> "SIN" And Origin <> "KTM" And Origin <> "MCT" And Origin <> "BKK" And Origin <> "DXB" And Origin <> "IXA" And Origin <> "IMF" And Origin <> "SXR" And Origin <> "IXJ" And Origin <> "DIB" And Origin <> "GAU" Then
                                                    ...
                                                    End If
                                                    bf = (Val(row("BaseAmt_Chd")) * 4.95) / 100
                                                Else
                                                    dTaxAmount_Chd = dTaxAmount_Chd + xmlDoc.GetElementsByTagName("AirComponents").ItemOf(0).ChildNodes.ItemOf(i).ChildNodes.ItemOf(1).ChildNodes.ItemOf(i1).ChildNodes.ItemOf(iCtr).ChildNodes.ItemOf(i3).ChildNodes.ItemOf(1).ChildNodes.ItemOf(0).InnerText
                                                End If
                                            Next
                                            'tax4 = Math.Round((((tax2 * 4.95) / 100) + ((tax3 * 4.95) / 100) + ((tax5 * 4.95) / 100) + bf), 0)
                                            tax4 = Math.Round(((tax2 * 4.95) / 100), 0) + Math.Round(((tax3 * 4.95) / 100), 0) + Math.Round(((tax5 * 4.95) / 100), 0) + Math.Round(bf, 0)
                                            row("TaxAmt_Chd") = dTaxAmount_Chd + tax2 + tax3 + tax4 + tax5
                                            row("TotalAmt_Chd") = (Val(row("TaxAmt_Chd")) + Val(row("BaseAmt_Chd"))).ToString
    ...
    Private Function GetAvailDT(ByVal dtAvail As DataTable, ByVal dtFareQuote As DataTable, ByVal sAirline As String, ByVal Adult As Integer, _
        ...
        Try
            ...
            For i As Integer = 0 To dtAvail.Rows.Count - 1
                ...
                row("ValiDatingCarrier") = sCarrierAccount
    ...
    Private Function GetFareQuoteReq(ByVal dtTable As DataTable, ByVal sClientID As String, ByVal sCarrierAccount As String, ByVal iTotalNo_Adt As Integer, ByVal iTotalNo_Chd As Integer, ByVal iTotalNo_Inf As Integer, ByVal iFareNo As Integer) As String
        ...
        Try
            ...
            While i <= dtTable.Rows.Count - 1
                ...
                '''''''''''''''''''
                Dim j As Integer = 0
                While j <= fltArray.Length - 1
                    fltdt.Clear()
                    fltdt.ImportRow(fltArray(j))
                    sXML += "<item xsi:type='ns1:Flight'> "
                    ' sXML += "<Status xsi:type='xsd:string'>SS</Status>"
                    If fltArray.Length > 2 Then
                        If ((fltArray(0)("FlightNumber")) = (fltArray(1)("FlightNumber"))) And ((fltArray(0)("FlightNumber")) = (fltArray(2)("FlightNumber"))) Then
                            sXML += "<ArrivalDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledArrivalTime")) & "</ArrivalDateTime>"
                            sXML += "<OpSuffix xsi:type='xsd:string' />"
                            sXML += "<NumberOfStops xsi:type='xsd:int'>" & fltdt.Rows(0)("Stops") & "</NumberOfStops>"
                            sXML += "<DepartureDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledDepartureTime")) & "</DepartureDateTime>"
                            sXML += "<Origin xsi:type='xsd:string'>" & (fltArray(j)("Origin")) & "</Origin>"
                            sXML += "<Destination xsi:type='xsd:string'>" & (fltArray(j)("Destination")) & "</Destination>"
                            sXML += "<CarrierCode xsi:type='xsd:string'>" & fltdt.Rows(0)("CarrierCode") & "</CarrierCode>"
                            sXML += "<FlightNumber xsi:type='xsd:string'>" & fltdt.Rows(0)("FlightNumber") & "</FlightNumber>"
                            sXML += "</item>"
                            Exit While
                        Else
                            If j + 1 <= fltArray.Length - 1 Then
                                If (fltArray(j)("FlightNumber")) <> (fltArray(j + 1)("FlightNumber")) Then
                                    sXML += "<ArrivalDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledArrivalTime")) & "</ArrivalDateTime>"
                                    sXML += "<OpSuffix xsi:type='xsd:string' />"
                                    sXML += "<NumberOfStops xsi:type='xsd:int'>" & fltdt.Rows(0)("Stops") & "</NumberOfStops>"
                                    sXML += "<DepartureDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledDepartureTime")) & "</DepartureDateTime>"
                                    sXML += "<Origin xsi:type='xsd:string'>" & (fltArray(j)("DepartureAirport")) & "</Origin>"
                                    sXML += "<Destination xsi:type='xsd:string'>" & (fltArray(j)("ArrivalAirport")) & "</Destination>"
                                    sXML += "<CarrierCode xsi:type='xsd:string'>" & (fltArray(j)("CarrierCode")) & "</CarrierCode>"
                                    sXML += "<FlightNumber xsi:type='xsd:string'>" & (fltArray(j)("FlightNumber")) & "</FlightNumber>"
                                    sXML += "</item>"
                                    j += 1
                                Else
                                    sXML += "<ArrivalDateTime xsi:type='xsd:dateTime'>" & (fltArray(j + 1)("ScheduledArrivalTime")) & "</ArrivalDateTime>"
                                    sXML += "<OpSuffix xsi:type='xsd:string' />"
                                    sXML += "<NumberOfStops xsi:type='xsd:int'>" & fltdt.Rows(0)("Stops") & "</NumberOfStops>"
                                    sXML += "<DepartureDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledDepartureTime")) & "</DepartureDateTime>"
                                    sXML += "<Origin xsi:type='xsd:string'>" & (fltArray(j)("DepartureAirport")) & "</Origin>"
                    ...
                    ElseIf fltArray.Length > 1 Then
                        If (fltArray(0)("FlightNumber")) = (fltArray(1)("FlightNumber")) Then
                            sXML += "<ArrivalDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledArrivalTime")) & "</ArrivalDateTime>"
                            sXML += "<OpSuffix xsi:type='xsd:string' />"
                            sXML += "<NumberOfStops xsi:type='xsd:int'>" & fltdt.Rows(0)("Stops") & "</NumberOfStops>"
                            sXML += "<DepartureDateTime xsi:type='xsd:dateTime'>" & (fltArray(j)("ScheduledDepartureTime")) & "</DepartureDateTime>"
    ...
    Private Function GetBookingReq(ByVal dtTemp As DataTable, ByVal custinfo As Hashtable, ByVal TotalFare As String, _
        ...
        Dim iTotalpass As Integer = iTotalNo_Adt + iTotalNo_Chd
        Dim sXML As String = "<?xml version='1.0' encoding='UTF-8'?>"
        sXML += "<SOAP-ENV:Envelope xmlns:SOAP-ENV='http://schemas.xmlsoap.org/soap/envelope/' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema'>"
        sXML += "<SOAP-ENV:Header>"
        sXML += "<ClientId>" & sClientID & "</ClientId>"
        sXML += "<DistributorNumber>" & sDistNo & "</DistributorNumber>"
        sXML += "<CarrierAccount>" & sCarrierAccount & "</CarrierAccount>"
        sXML += "<TraceId>" & dtTemp.Rows(0)("sno") & "</TraceId>"
        'sXML += "<ApiSource>API6E</ApiSource>"
        'sXML += "<AuthorizationKey></AuthorizationKey>"
        sXML += "</SOAP-ENV:Header>"
        sXML += "<SOAP-ENV:Body>"
        sXML += "<ns1:bookreservation xmlns:ns1='urn:os:bookreservation:v2' xmlns:ns2='http://xml.apache.org/xml-soap' SOAP-ENV:encodingStyle='http://schemas.xmlsoap.org/soap/encoding/' xmlns:SOAP-ENV='http://schemas.xmlsoap.org/soap/envelope/'>"
        ...
        sXML += "<TotalAmount xsi:type='ns1:Currency'>"
        sXML += "<Amount xsi:type='xsd:double'>" & TotalFare & "</Amount>"
        sXML += "<CurrencyCode xsi:type='xsd:string'>INR</CurrencyCode>"
        sXML += "</TotalAmount>"
        ...
        sXML += "<CurrencyCode xsi:type='xsd:string'>INR</CurrencyCode>"
        sXML += "</BaseAmount>"
        sXML += "<EquivalentAmount xsi:type='ns1:Currency'>"
        sXML += "<Amount xsi:type='xsd:double'>" & TotalFare & "</Amount>"
        sXML += "<CurrencyCode xsi:type='xsd:string'>INR</CurrencyCode>"
        sXML += "</EquivalentAmount>"
        sXML += "<PaymentMethod xsi:type='ns1:Prepaid'>"
        sXML += "<PaymentCode xsi:type='xsd:string'>AG</PaymentCode>"
        sXML += "<Comment xsi:type='xsd:string'>AG payment</Comment>"
        sXML += "<AuthorizationCode xsi:type='xsd:string'></AuthorizationCode>"
        sXML += "<ReferenceNumber xsi:type='xsd:string'>" & sDistNo & "</ReferenceNumber>"
        sXML += "</PaymentMethod>"
        sXML += "</item>"
        sXML += "</Payments>"
        sXML += "<NumberPassengers xsi:type='xsd:int'>" & iTotalpass & "</NumberPassengers>"
        ...
        If iTotalNo_Chd > 0 Then
            For iCtr As Integer = 1 To iTotalNo_Chd
                sXML += "<item xsi:type='ns1:Passenger'>"
                sXML += "<TypeCode xsi:type='xsd:string'>CHD</TypeCode>"
                If custinfo("Title_CHD" & iCtr).ToString.Trim.ToUpper = "MR" Then
                    sXML += "<Gender xsi:type='xsd:string'>M</Gender>"
                Else
                    sXML += "<Gender xsi:type='xsd:string'>F</Gender>"
                End If
    ...
    Private Function GetTraceIDForIndigo() As String
