...
Partial Class SprReports_Reissue_ReissueReport
    ...
    Public Sub CheckEmptyValue()
        Try
            ...
            Else
                Status = ""
            End If
            pnrds = objDA.GetReIssueDetail(UserID, UserType, FromDate, ToDate, OrderID, PNR, PaxName, TicketNo, Air, AgentID, ExecID, Status, trip.Trim())
            'adp.Fill(pnrds)
            If Session("user_type").ToString = "AGENT" Then
                grd_paxstatusinfo.Columns(22).Visible = False
                'grd_paxstatusinfo.Columns(23).Visible = False
                'grd_paxstatusinfo.Columns(24).Visible = False
            End If
            Session("pnrds") = pnrds
    ...
    'Protected Sub grd_paxstatusinfo_RowDataBound(sender As Object, e As GridViewRowEventArgs)
    '    Dim lblpaymentmode As Label
    '    Dim lblPGCharges As Label

    '    If e.Row.RowType = DataControlRowType.DataRow Then
    '        lblpaymentmode = e.Row.FindControl("lblPaymentMode")
    '        lblPGCharges = e.Row.FindControl("lblPGCharges")
