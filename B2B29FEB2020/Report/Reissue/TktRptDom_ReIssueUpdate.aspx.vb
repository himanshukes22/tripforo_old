...
Partial Class SprReports_Reissue_TktRptDom_ReIssueUpdate
...
#End Region
    ...
    Protected Sub btn_Update_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Update.Click
        ...
        Try
            ...
            For Each row As GridViewRow In ReissueUpdateGrd.Rows
                ...
                Dim fareDtlDS, FltDs As New DataSet()
                ...
                GetMerchantKey(lblorderId.Trim())
                ...
                If (Status = 0) Then
                    ...
                    Try
                        If ReissueUpdateGrd.Rows.Count >= i Then
                            'AgencyDs = ST.GetAgencyDetails(userid) Private TotalFare, Sum As Double
                            GrdDS = ViewState("GrdDS")
                            If GrdDS.Tables(0).Rows.Count = 1 Then
                                Dim GridTD As DataTable = GrdDS.Tables(0)
                                Dim ReissueCharge As Double = 0
                                Dim txtfareDiff As Double = 0
                                Dim txtfsrvChage As Double = 0

                                If Not String.IsNullOrEmpty(Request("txt_Reissue_charge")) Then
                                    ReissueCharge = Convert.ToDouble(Request("txt_Reissue_charge").Trim())
                                End If

                                If Not String.IsNullOrEmpty(Request("txt_farediff")) Then
                                    txtfareDiff = Convert.ToDouble(Request("txt_farediff").Trim())
                                End If
                                If Not String.IsNullOrEmpty(Request("txt_Service_charge")) Then
                                    txtfsrvChage = Convert.ToDouble(Request("txt_Service_charge").Trim())
                                End If

                                Dim Sum As Double = ReissueCharge + txtfsrvChage + txtfareDiff
                                ...
                                If (Convert.ToDouble(objBalResp.VAL_ACCOUNT_TYPE_DETAIL(0).VAL_ACCOUNT_BALANCE) > Sum) Then
                                    ...
                                    If objDebResp IsNot Nothing Then
                                        If objDebResp.MESSAGE.Trim().ToLower().Contains("successfully execute") Then
                                            ...
                                            'ST.InsertTransactionRepot(txt_pnr.Value.Trim(), td_AgentID.InnerText, 0, ablBalance.ToString(), Sum.ToString, sector, paxname, GridTD, td_AgencyName.InnerText, rm, StatusClass.Ticketed)                                            
                                            ...
                                            STDom.insertLedgerDetails(td_AgentID.InnerText, td_AgencyName.InnerText, NewOrderID, txt_pnr.Value.Trim(), txtTktNo.Value.Trim(), "", "", "", Session("UID").ToString, Request.UserHostAddress, _
                                                                Convert.ToDecimal(Sum), 0,
                IIf(objBalResp IsNot Nothing, IIf(objBalResp.VAL_ACCOUNT_TYPE_DETAIL.Length > 0, Convert.ToDecimal(objBalResp.VAL_ACCOUNT_TYPE_DETAIL(0).VAL_ACCOUNT_BALANCE), 0), 0), "Ticket ReIssue", rm, Convert.ToInt32(PaxID), ProjectId, BookedBy, CORPBILLNO)
                                            ...
                                            If HdrDs.Tables(0).Rows.Count > 0 Then
                                                ...
                                                'Dim CORPBILLNO As String = Nothing
                                                'If (HdrTD.Rows(0)("BillNoCorp") IsNot Nothing AndAlso HdrTD.Rows(0)("BillNoCorp") <> "") Then
    ...
    Private Sub BindGrid()
        ...
        End Try
    End Sub
    Private Sub InsertFlightDetails(ByVal FltTD As DataTable, ByVal GridTD As DataTable, ByVal NewOrderID As String)
