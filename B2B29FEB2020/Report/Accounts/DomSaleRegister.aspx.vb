...
Partial Class Reports_Accounts_DomSaleRegister
    ...
    Public Sub CheckEmptyValue()
        Try
            ...
            grdds.Clear()
            grdds = ST.IntGetInvoice(Session("UID").ToString, Session("User_Type").ToString, FromDate, ToDate, OrderID, PNR, TicketNo, AgentID, "D", Airline, ProjectId)
            'BindGrid(grdds)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                ...
                'Dim curr_date1 = Now()
                'grdds.Clear()
                'grdds = ST.IntGetInvoice(Session("UID").ToString, Session("User_Type").ToString, curr_date, curr_date1, Nothing, Nothing, Nothing, AgentID, "D", Nothing)
                'ViewState("grdds") = grdds
                'grd_IntsaleRegis.DataSource = grdds
                'grd_IntsaleRegis.DataBind()
            End If
    ...
    Public Sub ExportEmptyResult()
        Try
            ...
            If ToDate = "" Then
                ...
                strToDate = DateTime.Now.Date.ToString("dd/MM/yyyy")
                strToDate = strToDate.Substring(0, 2) & "/" & strToDate.Substring(3, 2) & "/" & strToDate.Substring(6, 4)

                If FromDate = "" Then
                    FromDate = DateTime.Now.ToString("dd/MM/yyyy")
                    strFromDate = FromDate.Substring(0, 2) & "/" & FromDate.Substring(3, 2) & "/" & FromDate.Substring(6, 4)
                    DTFrom = DateTime.ParseExact(strFromDate, "dd/MM/yyyy", CultureInfo.InvariantCulture)
                    ' DTFrom = Convert.ToDateTime(strFromDate)

                End If

                If FromDate <> DateTime.Now.ToString("dd/MM/yyyy").ToString() Then
                    strFromDate = FromDate.Substring(3, 2) & "/" & FromDate.Substring(0, 2) & "/" & FromDate.Substring(6, 4)
                    DTFrom = DateTime.ParseExact(strFromDate, "dd/MM/yyyy", CultureInfo.InvariantCulture)
                End If
                ' DTTo = Convert.ToDateTime(strToDate)
                DTTo = DateTime.ParseExact(strToDate, "dd/MM/yyyy", CultureInfo.InvariantCulture)
                TS = DTTo.Subtract(DTFrom)
            ElseIf ToDate <> "" And FromDate <> "" Then
                strFromDate = FromDate.Substring(3, 2) & "/" & FromDate.Substring(0, 2) & "/" & FromDate.Substring(6, 4)
                strToDate = ToDate.Substring(3, 2) & "/" & ToDate.Substring(0, 2) & "/" & ToDate.Substring(6, 4)
            ...
            End If
            NoOfDays = Convert.ToInt32(TS.TotalDays)
            If (NoOfDays <= 61) Then
                STDom.ExportData(grdds, "Dom._Sale_Register-From " & FromDate & "-To-" & ToDate)
            Else
                ScriptManager.RegisterStartupScript(Page, Page.[GetType](), "print", "alert('Selected date must be less than two months!!');", True)
            End If
