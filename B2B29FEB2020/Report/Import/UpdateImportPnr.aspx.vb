...
Partial Class Reports_Import_UpdateImportPnr
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not Page.IsPostBack Then
                ...
                ''ds = ST.ImportPNRDetailsIntl("Pending", "D", Request("OrderId"))
                '' defaultset()
                ds = ImportPNRDetailsIntl("Pending", "D", Request("OrderId"))
                ViewState("PnrDs") = ds
                ' Fltheadr_column
                ...
                Try
                    ...
                    ds1 = ST.PnrImportIntlDetails(Request("OrderId"), "D")
                    dt = ds1.Tables(0)
                    ...
                    Else
                        ...
                        If (ViewState("BookedBy") Is Nothing) Then
                            ViewState("BookedBy") = Nothing
                        Else
    ...
    Protected Sub btnCalc_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCalc.Click
        Try
            ...
            Dim CommADT As Double = 0, CommCHD As Double = 0, CommINF As Double = 0, TotalComm As Double = 0
            ...
            'Calculate Commission And Cash Back
            ds = ST.PnrImportIntlDetails(Request("OrderId"), "D")
            ...
            '''''For Service Tax And Transaction Fee''''
            ds1 = ST.calcServicecharge(VC, "D")
            dt = ds1.Tables(0)
            ...
            Dim CBChild As Double = 0
            Dim TotalCB As Double = 0
            Dim dtcommADT As DataTable
            ...
            If (IsCorp = True) Then
                'MARKUP
                dtgetmrk = ClsCorp.GetMarkUp(IntAirDt.Rows(0)("AgentID").ToString(), "SPRING", "D", "AD").Tables(0)
                If (dtgetmrk.Rows.Count > 0) Then
                    Adminmrkadt = ClsCorp.CalcMarkup(dtgetmrk, txtVC.Text.Trim, Convert.ToDouble(txt_ABaseFare.Text), "D")
            ...
            If (IsCorp = True) Then
                ...
                dtcommADT = ClsCorp.GetManagementFeeSrvTax(GroupType, txtVC.Text.Trim, Convert.ToDouble(txt_ABaseFare.Text), Convert.ToDouble(txt_AYQ.Text), "D", Convert.ToDouble(lblATotal.Text)).Tables(0)
                'If (dtcommADT.Rows.Count > 0) Then
                CommADT = Math.Round(Convert.ToDouble(dtcommADT.Rows(0)("MGTFEE").ToString()), 0) * ADT
                'End If

                CBAdult = 0 ' Math.Round(Convert.ToDouble(dtcommADT.Rows(0)("CB").ToString()), 0) * ADT

                If (CHD > 0) Then
                    dtcommCHD = ClsCorp.GetManagementFeeSrvTax(GroupType, txtVC.Text.Trim, Convert.ToDouble(txt_CBaseFare.Text), Convert.ToDouble(txt_CYQ.Text), "D", Convert.ToDouble(lblCTotal.Text)).Tables(0)
                    'If (dtcommCHD.Rows.Count > 0) Then
                    CommCHD = Math.Round(Convert.ToDouble(dtcommCHD.Rows(0)("MGTFEE").ToString()), 0) * CHD
                    'End If

                    CBChild = 0 'Math.Round(Convert.ToDouble(dtcommCHD.Rows(0)("CB").ToString()), 0) * CHD
                    'MARKUP
                    If (dtgetmrk.Rows.Count > 0) Then
                        Adminmrkchd = ClsCorp.CalcMarkup(dtgetmrk, txtVC.Text.Trim, Convert.ToDouble(txt_CBaseFare.Text), "D")
                ...
                End If
                SrviTax = STaxADT + STaxCHD + STaxINF

                '''''''''''''''''''''''''''End Calculation ServiceTax''''''''''''''''''''''''''''

            Else
                tr_admrkmgt.Visible = False
                dtcommADT = CCAP.calcCommDom(GroupType, txtVC.Text.Trim, Convert.ToDouble(txt_ABaseFare.Text), Convert.ToDouble(txt_AYQ.Text), 1)
                CommADT = Math.Round(Convert.ToDouble(dtcommADT.Rows(0)("Dis").ToString()), 0) * ADT
                CBAdult = Math.Round(Convert.ToDouble(dtcommADT.Rows(0)("CB").ToString()), 0) * ADT
                CommADT1 = CommADT '*****
                If (CHD > 0) Then
                    dtcommCHD = CCAP.calcCommDom(GroupType, txtVC.Text.Trim, Convert.ToDouble(txt_CBaseFare.Text), Convert.ToDouble(txt_CYQ.Text), 1)
                    CommCHD = Math.Round(Convert.ToDouble(dtcommCHD.Rows(0)("Dis").ToString()), 0) * CHD
                    CBChild = Math.Round(Convert.ToDouble(dtcommCHD.Rows(0)("CB").ToString()), 0) * CHD
                    CommCHD1 = CommCHD '*****
                End If
                ...
                '''''''''''''''''''Calculate ServiceTax'''''''''''''''''''''''''''''''''''''
                STaxPerADT = Math.Round(((dtcommADT.Rows(0)("Dis").ToString() * STax) / 100), 0)
                STaxADT = Math.Round((STaxPerADT), 0) * ADT
                STaxADT1 = STaxADT '*****
                STaxPerADT1 = STaxPerADT
                If (CHD > 0) Then
                    STaxPerCHD = Math.Round(((dtcommCHD.Rows(0)("Dis").ToString() * STax) / 100), 0)
                    STaxCHD = Math.Round((STaxPerCHD), 0) * CHD
                    STaxCHD1 = STaxCHD1 '*****
            ...
            If (IsCorp = True) Then
                ...
                lblTtlDis.Text = 0
                lblTtlCB.Text = TotalCB
                lblTds.Text = TdsOn
                lbl_admrk.Text = (Adminmrkadt + Adminmrkchd)
            Else
                tdsper = CCAP.geTdsPercentagefromDb(IntAirDt.Rows(0)("AgentID").ToString)
                TDSPerADT = ((Convert.ToDouble(dtcommADT.Rows(0)("Dis").ToString) - STaxPerADT1) * Convert.ToDouble(tdsper)) / 100
                TDSADT = Math.Round((TDSPerADT), 0) * ADT
                TDSPerADT1 = ((Convert.ToDouble(dtcommADT.Rows(0)("Dis").ToString)) * Convert.ToDouble(tdsper)) / 100
                TDSADT1 = Math.Round((TDSPerADT1), 0) * ADT

                If (CHD > 0) Then
                    TDSPerCHD = ((Convert.ToDouble(dtcommCHD.Rows(0)("Dis").ToString) - STaxPerCHD1) * Convert.ToDouble(tdsper)) / 100
                    TDSCHD = Math.Round((TDSPerCHD), 0) * CHD
                    TDSPerCHD1 = ((Convert.ToDouble(dtcommCHD.Rows(0)("Dis").ToString)) * Convert.ToDouble(tdsper)) / 100
                ...
                lblSrvTax.Text = SrviTax
                lblTF.Text = TFee
                lbltfAftrDis.Text = (Convert.ToDouble(lblTtlFare.Text) + SrviTax + TFee + TdsOn) - (TotalComm + TotalCB)
        ...
        If (ViewState("Status") = "NotActive") AndAlso Status = False Then
            ...
            Try
                ...
                If CrdLimit > Convert.ToDouble(lbltfAftrDis.Text) + Convert.ToDouble(txtpnrImpCharge.Text.Trim) + ImportCharge Then
                    ...
                    If (IsCorp = True) Then
                        CORPBILLNO = ClsCorp.GenerateBillNoCorp("D").ToString()
                    End If
                    ...
                    If txtpnrImpCharge.Text <> "0" Then
                        ...
                        If (CheckBalStatusIMP = False) Then
                            STDom.insertLedgerDetails(dt.Rows(0)("AgentID").ToString, dt.Rows(0)("Ag_Name").ToString, Request("OrderId"), "", "", "", "", "", Session("UID").ToString, Request.UserHostAddress, txtpnrImpCharge.Text, 0, A_Bal, "ImoprtChargeDom", "ES Charge with OrderId: " & Request("OrderId") & "Pnr:" & lblGdsPnr.Text, 0, ViewState("ProjectId"), ViewState("BookedBy"), CORPBILLNO)
                        End If
                    End If
                    ...
                    If (CheckBalStatus = False AndAlso CheckBalStatusIMP = False) Then
                        ...
                        For i As Integer = 0 To dt.Rows.Count - 1
                            Try
                                ...
                                ''ST.
                                insertFlightDetailsPnrImport(Request("OrderId"), dt.Rows(i)("Departure").ToString.Trim, _
                                AirlineNameDepart, dt.Rows(i)("Destination").ToString.Trim, AirNameDest, _
                                dt.Rows(i)("DepartDate").ToString.Trim, dt.Rows(i)("DepartTime").ToString.Trim, _
                                dt.Rows(i)("ArrivalDate").ToString, dt.Rows(i)("ArrivalTime").ToString.Trim, dt.Rows(i)("Airline").ToString.Trim, _
                                AirlineName, dt.Rows(i)("FlightNo").ToString.Trim, "", dt.Rows(i)("AdtFareBasis").ToString, dt.Rows(i)("ChdFareBasis").ToString, dt.Rows(i)("InfFareBasis").ToString, dt.Rows(i)("RDB").ToString, dt.Rows(i)("RDB").ToString, dt.Rows(i)("RDB").ToString, ADT, CHD, INF)
                            Catch ex As Exception
                                clsErrorLog.LogInfo(ex)
                            End Try
                        Next
                        ...
                        ''BULK END
                                ...
                                For Each rw In paxRPTR.Items
                                    ...
                                    If ddltitle.SelectedValue = "ADT" Then
                                        ...
                                        STDom.insertLedgerDetails(dt.Rows(0)("AgentID").ToString, dt.Rows(0)("Ag_Name").ToString, Request("OrderId"), lblGdsPnr.Text.Trim, txtTkt.Text, txtVC.Text.Trim, "", "", Session("UID").ToString, IP, DebitADT, 0, AvalBalance, "ImoprtDom", "Created By PNR Import Dom", 0, ViewState("ProjectId"), ViewState("BookedBy"), CORPBILLNO)
                                    End If
                                    If (CHD > 0) Then
                                        If ddltitle.SelectedValue = "CHD" Then
                                            ...
                                            STDom.insertLedgerDetails(dt.Rows(0)("AgentID").ToString, dt.Rows(0)("Ag_Name").ToString, Request("OrderId"), lblGdsPnr.Text.Trim, txtTkt.Text, txtVC.Text.Trim, "", "", Session("UID").ToString, IP, DebitCHD, 0, AvalBalance, "ImoprtDom", "Created By PNR Import Dom", 0, ViewState("ProjectId"), ViewState("BookedBy"), CORPBILLNO)
                                    ...
                                    If (INF > 0) Then
                                        If ddltitle.SelectedValue = "INF" Then
                                            If (DtFltFare.Rows(1)("PaxType").ToString() = "INF") Then
                                                ...
                                                STDom.insertLedgerDetails(dt.Rows(0)("AgentID").ToString, dt.Rows(0)("Ag_Name").ToString, Request("OrderId"), lblGdsPnr.Text.Trim, txtTkt.Text, txtVC.Text.Trim, "", "", Session("UID").ToString, IP, DebitINF, 0, AvalBalance, "ImoprtDom", "Created By PNR Import Dom", 0, ViewState("ProjectId"), ViewState("BookedBy"), CORPBILLNO)
                                            Else
                                ...
                                '    STYTR.InsertYatra_MIRHEADER(Request("OrderId"), lblGdsPnr.Text.Trim)
                                '    STYTR.InsertYatra_PAX(Request("OrderId"), lblGdsPnr.Text.Trim)
                                '    ST YTR.InsertYatra_SEGMENT(Request("OrderId"), lblGdsPnr.Text.Trim)
                                '    STYTR.InsertYatra_FARE(Request("OrderId"), lblGdsPnr.Text.Trim)
                                '    STYTR.InsertYatra_DIFTLINES(Request("OrderId"), lblGdsPnr.Text.Trim)
                            ...
                            ''objSql.SmsLogDetails(Request("OrderId"), lbl_mn.Text, smsMsg, smsStatus)
                                ...
                                ClientScript.RegisterStartupScript(Page.GetType(), "Alert", "alert('Pnr Imported Sucessfully');window.location='ProcessImportPnr.aspx';", True)
    ...
    Public Sub CalFareDetails(ByVal Orderid As String, ByVal VC As String, ByVal PaxType As String, ByVal BaseFare As Integer, ByVal YQ As Integer, ByVal YR As Integer, ByVal WO As Integer, ByVal GST As Integer, ByVal OT As Integer, ByVal Adminmrk As Double)
        Try
            ...
            If (IsCorp = True) Then
                ...
                ds = ST.PnrImportIntlDetails(Orderid, "D")
                IntAirDt = ds.Tables(0)
                ...
                dtcom = ClsCorp.GetManagementFeeSrvTax(GroupType, VC, BaseFare, YQ, "D", (BaseFare + YQ + YR + WO + OT + GST)).Tables(0)
                If dtcom.Rows.Count > 0 Then
                    Commcal = dtcom.Rows(0)("MGTFEE").ToString()
                    CBCal = 0 'dtcom.Rows(0)("CB").ToString()
                ...
                tdsper = 0 'Convert.ToDouble(CCAP.geTdsPercentagefromDb(IntAirDt.Rows(0)("AgentID").ToString))
                'End If

                Tds = 0 'Math.Round((Commcal * Convert.ToDouble(tdsper) / 100), 0)
                ' End If
                ClsCorp.calcFareCorp(Tax.Split("#"), Orderid, PaxType, BaseFare, Adminmrk, 0, 0, Comm, 0, srvtax, 0, 0, VC, "D")
                '=====================================================END CORPORATE==============================================
            Else
                ...
                If PaxType <> "INF" Then
                    ...
                    dtsrv = ST.calcServicecharge(VC, "D").Tables(0)
                    ...
                    'Cal TDS
                    If IntAirDt.Rows.Count > 0 Then
                        tdsper = Convert.ToDouble(CCAP.geTdsPercentagefromDb(IntAirDt.Rows(0)("AgentID").ToString))
                    End If

                    Tds = Math.Round((Commcal * Convert.ToDouble(tdsper) / 100), 0)
                End If

                ''ST.calcFare(Tax.Split("#"), Orderid, PaxType, BaseFare, 0, 0, 0, Comm, CBCal, Tds, VC, "D", Comm1, "")
                calcFare(Tax.Split("#"), Orderid, PaxType, BaseFare, 0, 0, 0, Comm, CBCal, Tds, VC, "D", Comm1, "")
            End If
