...
Partial Class Reports_Import_PnrImportStatusIntl
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            Try
                ...
                'pnrds.Clear()
                'pnrds = ST.ImportPnrReport_Intl(Session("User_Type"), Session("UID").ToString, curr_date, curr_date1, OrderID, PNR, AirPNR, AgentID, "I", ExecID, Status)
    ...
    Protected Sub btn_result_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_result.Click
        Try
            ...
            'string FromDate = String.IsNullOrEmpty(From.Text) ? null : From.Text.Split(" ")(0);
            ' Dim ToDate As String = If([String].IsNullOrEmpty([To].Text), Nothing, [To].Text)
            Dim OrderID As String = If([String].IsNullOrEmpty(txt_OrderId.Text), Nothing, txt_OrderId.Text.Trim)
            Dim PNR As String = If([String].IsNullOrEmpty(txt_PNR.Text), Nothing, txt_PNR.Text.Trim)
            Dim AirPNR As String = If([String].IsNullOrEmpty(txt_AirPNR.Text), Nothing, txt_AirPNR.Text.Trim)
            Dim ExecID As String
            ...
            pnrds.Clear()
            pnrds = ST.ImportPnrReport_Intl(Session("User_Type"), Session("UID").ToString, FromDate, ToDate, OrderID, PNR, AirPNR, AgentID, "I", ExecID, Status)
            Session("pnrds") = pnrds
            BindDate(pnrds)
        Catch ex As Exception
            clsErrorLog.LogInfo(ex)
    ...
    Sub BindDate(ByVal ImportPnrDs As DataSet)
        Try
            ...
            For Each rowitem As GridViewRow In GridImportProxyDetail.Rows
                ...
                DirectCast(rowitem.FindControl("ancher"), HtmlAnchor).HRef = "ImportPnrDetailsIntl.aspx?OrderId=" & (pnrArray(0)("OrderId")).ToString
