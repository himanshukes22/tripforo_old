...
Partial Class SprReports_Distr_UploadRequest
        ...
        If Not IsPostBack Then
            Try
                ...
                If Session("UID") <> "" AndAlso Session("UID") IsNot Nothing Then
                    grd_accdeposit.DataSource = Distr.DepositStatusDetails("DIPending", Session("UID")) ' upload.GetDepositProcessDetails("DIPending")
    ...
    Protected Sub grd_accdeposit_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs)
        Try
            ...
            If e.CommandName = "accept" Then
                id = e.CommandArgument.ToString()
                STDom.UpdateDepositDetails(ID, AgentID.Text, "DIInProcess", "Acc", Session("UID"), txt_Reject.Text.Replace("'", ""))
                'ClientScript.RegisterStartupScript(Page.GetType(), "Alert", "alert('Rejected Sucessfully');", True)
                'td_Reject.Visible = False
                grd_accdeposit.DataSource = Distr.DepositStatusDetails("DIPending", Session("UID"))
                grd_accdeposit.DataBind()
            End If

            If e.CommandName = "reject" Then

                If IDS = ID AndAlso Status = "DIConfirm" Then
    ...
    '        dtgrid = STDom.DepositProcessDetails("DIPending", ddl_Category.SelectedValue).Tables(0)
    ...
    '            Dim dt As DataTable = STDom.DepositStatusDetails("DIPending").Tables(0)
    ...
    '            grd_accdeposit.DataBind()
    '        Else
    '            Dim dt As DataTable = STDom.DepositProcessDetails("DIPending", ddl_Category.SelectedValue).Tables(0)
    '            Dim dv As New DataView(dt)
    '            Dim SearchExpression As String = Nothing
    '            If Not [String].IsNullOrEmpty(txtSearch.Text) Then
    ...
    Protected Sub btn_Submit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Submit.Click
        Try
            STDom.UpdateDepositDetails(ViewState("ID"), ViewState("AgentID"), "DIADRejected", "Rej", Session("UID"), txt_Reject.Text.Replace("'", ""))
            ClientScript.RegisterStartupScript(Page.GetType(), "Alert", "alert('Rejected Sucessfully');", True)
            td_Reject.Visible = False
            grd_accdeposit.DataSource = Distr.DepositStatusDetails("DIPending", Session("UID"))
            grd_accdeposit.DataBind()
    ...
    Protected Sub btn_AgencySubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_AgencySubmit.Click
        Dim dsag As New DataSet
        dsag = Distr.GetDepositeDetailsById(Request("hidtxtAgencyName"), "DIPending")
