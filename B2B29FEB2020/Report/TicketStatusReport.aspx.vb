...
Partial Class SprReports_TicketStatusReport
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                End If
                grdds.Clear()
                grdds = ST.USP_GetTicketDetail_IntlStatus(Session("UID").ToString, Session("User_Type").ToString, "", "", "", "", "", "", "", AgentID, trip.Trim(), "Ticketed")
                'BindGrid(grdds)
                Dim dt As DataTable
                Dim Db As String = ""
                Dim sum As Double = 0
    ...
    Private Sub BindGrid(ByVal Gds As DataSet)
        ...
        Try
            ...
            End If
            ticket_grdview.Columns(1).Visible = False
            ...
            '    'ticket_grdview.Columns(20).Visible = True
            '    'ticket_grdview.Columns(6).Visible = True
            '    'ticket_grdview.Columns(17).Visible = False
            'Else
            '    'ticket_grdview.Columns(20).Visible = False
        ...
        Try
            ...
            ElseIf (Request("RemarksType").Trim() = "REISSUE") Then
                ...
                If fltds.Tables(0).Rows.Count <> 0 Then
                    ...
                    Dim Status As String = CheckTktNo(Convert.ToInt32(Request("txtPaxid").Trim()), fltTD.Rows(0).Item("OrderId"), fltTD.Rows(0).Item("PNR"))
                    If Status = "0" Then
                        Dim i As Integer = 0
                        ...
                        If (Request("RemarksType").Trim() = "REISSUE") Then
                            ...
                            Try
                                If Session("LoginByOTP") IsNot Nothing AndAlso Convert.ToString(Session("LoginByOTP")) <> "" AndAlso Convert.ToString(Session("LoginByOTP")) = "true" Then
                                    ...
                                    Dim OTPRefNo As String = fltTD.Rows(0).Item("OrderId")
                                    Dim LoginByOTP As String = Session("LoginByOTP")
                                    Dim OTPId As String = Session("OTPID")
                                    Dim ServiceType As String = "FLIGHT-TICKET" + Request("RemarksType").Trim()
                                    Dim flag As Integer = 0
                                    flag = ST.OTPTransactionInsert(UserId, Remark, OTPRefNo, LoginByOTP, OTPId, ServiceType)
                                End If
                            Catch ex As Exception
                                'clsErrorLog.LogInfo(ex)
                                EXCEPTION_LOG.ErrorLog.FileHandling("TicketReport", "Error_102", ex, "SprReports-TicketReport.aspx.vb-OTPTransactionInsert-btnRemark_Click")
                            End Try
    ...
    Public Sub CheckEmptyValue()
        Try
            ...
            End If
            grdds.Clear()
            grdds = ST.USP_GetTicketDetail_IntlStatus(Session("UID").ToString, Session("User_Type").ToString, FromDate, ToDate, OrderID, PNR, PaxName, TicketNo, AirPNR, AgentID, trip.Trim(), dd_status.SelectedValue)
            BindGrid(grdds)
        ...
        Try
            ...
            Else
                ...
                If grdds.Tables(0).Columns.Contains("PartnerName") = True Then
                    ...
                    StrDataTable.Columns.Remove("rownum")
                    StrDataTable.Columns.Remove("rownum1")
                    StrDataTable.Columns.Remove("AgentType")
                    StrDataTable.Columns.Remove("Paxid")
                ...
                If Status = "0" Or Status = "Reissue request can not be accepted for past departure date." Or Status = "Refund/Reissue request can not be accepted upto 4 hour prior to departure date. Please contact to airline directly." Then
                    If fltds.Tables(0).Rows.Count <> 0 Then
                        ...
                        If Status = "0" Or Status = "Reissue request can not be accepted for past departure date." Or Status = "Given ticket number is already ReIssued" Or Status = "Refund/Reissue request can not be accepted upto 4 hour prior to departure date. Please contact to airline directly." Then
                            ...
                            Catch ex As Exception
                                'clsErrorLog.LogInfo(ex)
                                EXCEPTION_LOG.ErrorLog.FileHandling("TicketReport", "Error_102", ex, "SprReports-TicketReport.aspx.vb-OTPTransactionInsert")
                            End Try
